-- Keep distinct real product families, but suppress lower-quality duplicate imports
-- of the same active Registry release. Ties remain visible and therefore fail closed.
...
as $$
  with release_quality as materialized (
    select
      r.id,
      v.id as version_id,
      coalesce(r.release_year, substring(r.season from '(19|20)[0-9]{2}')) as release_key_year,
      trim(
        regexp_replace(
          regexp_replace(
            case
              when m.name is not null then replace(lower(coalesce(r.product_name, '')), lower(m.name), ' ')
              else lower(coalesce(r.product_name, ''))
            end,
            '(19|20)[0-9]{2}|basketball',
            ' ',
            'g'
          ),
          '[^a-z0-9]+',
      ...
      (
        case when r.brand_id is not null then 2 else 0 end +
        case when r.league_id is not null then 2 else 0 end +
        case when r.season is not null then 1 else 0 end +
        case when r.release_year is not null and lower(coalesce(r.product_name, '')) like '%' || lower(r.release_year) || '%' then 1 else 0 end +
        case when m.name is not null and lower(coalesce(r.product_name, '')) like '%' || lower(m.name) || '%' then 1 else 0 end +
        case when b.name is not null and lower(coalesce(r.product_name, '')) like '%' || lower(b.name) || '%' then 1 else 0 end +
        case when l.name is not null and lower(coalesce(r.product_name, '')) like '%' || lower(l.name) || '%' then 1 else 0 end
      ) as release_quality
  ...
  release_ranked as materialized (
    select
      rq.*,
      dense_rank() over (
        partition by rq.release_key_year,
          case when rq.release_family = '' then rq.id::text else rq.release_family end
        order by rq.release_quality desc
      ) as release_rank
    from release_quality rq
  ),
  player_cards as materialized (
  ...
  matched_cards as materialized (
    ...
    join release_ranked rr on rr.id = c.release_id
      and rr.version_id = c.version_id
