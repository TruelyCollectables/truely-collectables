-- InstaComp exact-card longitudinal market history.
-- Automated writes are accepted only after the caller has a canonical Checklist
-- Registry identity ID + fingerprint. Observations are append-only: prices are
-- never updated in place, so historical market direction remains reconstructable.
...
comment on table public.tcos_card_market_identities is
  'Canonical Checklist Registry identities that have trusted longitudinal market history.';
