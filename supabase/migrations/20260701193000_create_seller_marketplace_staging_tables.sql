...
create index if not exists seller_marketplace_import_jobs_account_idx
  on public.seller_marketplace_import_jobs(account_id, store_id, provider, created_at desc);
...
create table if not exists public.seller_marketplace_staged_items (
  ...
  store_id uuid not null
    ...
    references public.stores(id) on delete cascade,
  connection_id uuid references public.seller_marketplace_connections(id) on delete set null,
  import_job_id uuid references public.seller_marketplace_import_jobs(id) on delete set null,
  provider text not null,
...
create index if not exists seller_marketplace_staged_items_account_idx
  on public.seller_marketplace_staged_items(account_id, store_id, provider, updated_at desc);
