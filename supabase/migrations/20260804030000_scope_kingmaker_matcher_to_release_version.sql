-- Match bounded Beckett candidate IDs directly against the one active
-- Checklist Registry release/version that triggered the rematch. This removes
-- the cross-release normalized join that timed out in Production.

begin;

create or replace function public.tcos_match_kingmaker_price_entry_ids_for_release(
  p_guide_id uuid,
  p_entry_ids uuid[],
  p_release_id uuid,
  p_version_id uuid
...
begin
  if coalesce(cardinality(p_entry_ids), 0) = 0 then
    return jsonb_build_object(
      ...
      'review', 0,
      'unmatched', 0,
      'scope', 'release_version_entry_ids'
    );
  end if;

  if not exists (
    select 1
    from public.checklist_versions version
    where version.id = p_version_id
      and version.release_id = p_release_id
      and version.is_active
      and version.status in ('live', 'revised')
  ) then
    raise exception
      'Checklist version % is not the active live/revised version for release %',
      p_version_id,
      p_release_id;
  end if;
  ...
  with candidate_rows as (
    ...
    from unnest(p_entry_ids) as target(entry_id)
    join public.tcos_kingmaker_price_entries entry
      on entry.id = target.entry_id
     and entry.guide_id = p_guide_id
    join public.checklist_sets set_row
      on set_row.release_id = p_release_id
     and set_row.version_id = p_version_id
     and (
       set_row.normalized_name =
    ...
    join public.checklist_cards card
      on card.release_id = p_release_id
     and card.version_id = p_version_id
     and card.set_id = set_row.id
     and card.normalized_card_number =
         public.tcos_kingmaker_price_normalize(entry.card_number)
    join public.checklist_card_identities identity
      on identity.release_id = p_release_id
     and identity.version_id = p_version_id
  ...
  select
    ...
    jsonb_build_object(
      ...
      'match_scope', 'release_version_entry_ids',
      'release_id', p_release_id,
      'checklist_version_id', p_version_id
    )
  from unnest(p_entry_ids) as target(entry_id)
...
begin
  ...
  if definition is null then
    raise exception 'KINGMAKER batched release rematch function is missing.';
  end if;
  ...
  if position(
    ...
    patched_definition := regexp_replace(
      patched_definition,
      'select[[:space:]]+public\.tcos_match_kingmaker_price_entry_ids\([[:space:]]*current_guide_id,[[:space:]]*candidate_entry_ids[[:space:]]*\)[[:space:]]+into[[:space:]]+matcher_result;',
      'select public.tcos_match_kingmaker_price_entry_ids_for_release(current_guide_id, candidate_entry_ids, release_row.release_id, release_row.active_version_id) into matcher_result;',
      'g'
    );
  end if;
  ...
  if position(
    ...
    raise exception 'Cross-release candidate matcher call remains after direct release/version patch.';
  end if;
  ...
  if position(
    ...
    raise exception 'Direct release/version candidate matcher call was not installed.';
  end if;
...
revoke all on function public.tcos_match_kingmaker_price_entry_ids_for_release(uuid, uuid[], uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.tcos_match_kingmaker_price_entry_ids_for_release(uuid, uuid[], uuid, uuid)
  to service_role;
