...
- EXTREMELY HOT top priority after the current InstaComp™ accuracy pass: build the `TCOS Card Knowledge Base MVP`.
  ...
  - Clickable comp-evidence rule: batch rows show `InstaComp™ Suggested` as the primary price, smaller `Market guidance` underneath, and a clickable usable-comp count/range. Expanding it shows each usable comp as a link to the source item with source, basis, match score, and price. Future pricing freshness should store sale/listing timestamps so stale comps can be excluded or down-weighted.
