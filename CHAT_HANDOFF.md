...
- EXTREMELY HOT top priority after the current InstaComp™ accuracy pass: build the `TCOS Card Knowledge Base MVP`.
  ...
  - Clickable comp-evidence/current-pricing rule: batch rows show `InstaComp™ Suggested` as the primary price, smaller `Market guidance` underneath, and a clickable usable-comp count/range. Expanding it shows each usable comp as a link to the source item with source, basis, match score, freshness, and price. Dated sold comps older than the current-pricing freshness window stay visible as historical evidence but do not factor into `InstaComp™ Suggested`; undated sold comps remain usable until provider sale dates are available.
  - Seller eBay one-click import rule: Seller Connections now exposes `1-Click Bring eBay Listings to TCOS` for staging all remaining connected eBay listings without making anything live automatically. Selected staged rows can be sent to InstaComp™ through the bulk `Send Selected to InstaComp™` handoff or row-level `Send to InstaComp™` links; `/admin/instacomp` shows a seller eBay staging context banner when opened from those links.
  - Admin login diagnostic rule: `/api/admin/login` now returns stable `code` values for missing admin password, invalid password, lockout, and blocked-client states, and `/admin/login` displays remaining attempts when available. If local `.env.local` changes, restart the dev server before retrying.
