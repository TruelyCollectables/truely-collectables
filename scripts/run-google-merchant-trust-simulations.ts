...
assert.equal(
  sanitizePublicListingTitle(
    "2025 Panini Prizm - Zduplicate Orange Ice Player #1",
  ),
...
assert.equal(
  sanitizePublicListingDescription(
    '<div class="ql-editor"><p>Clean <strong>listing</strong> copy &amp; details</p></div>',
  ),
  "Clean listing copy & details",
);

const bulkListingDescription = buildPublicListingDescription({
  title: "2021 Topps Chrome Update - Topps Black Gold Aaron Judge #BGC-10",
  description:
    "Name/Player: Aaron Judge Team: New York Yankees Year: 2021 Brand: Topps Set: 2021 Topps Chrome Update - Topps Black Gold Card number: BGC-10 Condition: RAW",
  player: "Aaron Judge",
  sport: "Baseball",
});
assert.match(bulkListingDescription, /Aaron Judge/);
assert.match(bulkListingDescription, /Welcome to Truely Collectables/);
assert.match(bulkListingDescription, /Tracked Card Letter/);
assert.doesNotMatch(bulkListingDescription, /eBay Standard Envelope/i);
...
for (const file of [
  ...
  "cloudflare-storefront-entrypoint.ts",
]) {
  const source = fs.readFileSync(file, "utf8");
  assert.ok(
    source.includes('url.protocol === "http:"'),
    `${file} must reject plaintext storefront delivery.`,
  );
  assert.ok(
    source.includes('url.protocol = "https:"'),
    `${file} must redirect plaintext requests to HTTPS.`,
  );
}
...
for (const requiredGoogleSource of [
