...
async function main() {
  ...
  const serviceRoleKey = productionEnv.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !serviceRoleKey) throw new Error('Production Supabase URL/service role key are missing.');
  ...
  const keyRows = await managementQuery({
    project,
    token: accessToken,
    readOnly: true,
    parameters: [TRANSPORT_ID],
    query: `select private_key_pem, expires_at, consumed_at from private.tcos_kingmaker_beckett_transport_keys where transport_id = $1`,
  });
  ...
  const wrappedKey = readFileSync(join(workDir, 'wrapped-key.bin'));
  const aesKey = privateDecrypt({ key: keyRow.private_key_pem, padding: constants.RSA_PKCS1_OAEP_PADDING, oaepHash: 'sha256' }, wrappedKey);
  ...
  const plaintextTar = Buffer.concat([decipher.update(ciphertext), decipher.final()]);
  ...
  for (const zipName of FILES) {
    const zipPath = join(bundlesDir, zipName);
  ...
  await managementQuery({
    ...
    readOnly: false,
    parameters: [TRANSPORT_ID],
    query: `update private.tcos_kingmaker_beckett_transport_keys set consumed_at = now() where transport_id = $1 and consumed_at is null`,
  });
  ...
  const receipt = {
    ...
    certifiedCorpus: { guides: 5, pages: EXPECTED_TOTAL_PAGES, rows: EXPECTED_TOTAL_ROWS },
    imports,
