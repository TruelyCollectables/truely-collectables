...
async function uploadRegistrySource(db, plan, bytes) {
  ...
  for (let attempt = 1; attempt <= UPLOAD_ATTEMPTS; attempt += 1) {
    try {
      ...
      lastError = new Error(uploaded.error.message || String(uploaded.error));
    } catch (error) {
      lastError = error instanceof Error ? error : new Error(String(error));
    }
    console.warn(`Registry source upload attempt ${attempt}/${UPLOAD_ATTEMPTS} failed: ${lastError.message}`);
    if (attempt >= UPLOAD_ATTEMPTS || !transientMessage(lastError.message)) break;
    await sleep(Math.min(30_000, 2_000 * 2 ** (attempt - 1)));
  }
  throw new Error(`Could not archive validated Registry source: ${lastError?.message || "unknown storage error"}`);
}
