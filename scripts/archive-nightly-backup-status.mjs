...
function assertEvidenceContract(payload) {
  ...
  if (typeof payload?.launchdRuntime?.loaded !== "boolean") {
    missing.push("launchdRuntime.loaded");
  }
  if (!payload?.launchdRuntime || !("runs" in payload.launchdRuntime)) {
    missing.push("launchdRuntime.runs");
  }
  if (!payload?.launchdRuntime || !("lastExitCode" in payload.launchdRuntime)) {
    missing.push("launchdRuntime.lastExitCode");
  }
