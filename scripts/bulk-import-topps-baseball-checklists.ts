...
const MAX_SOURCES = Math.max(
  1,
  Math.min(500, Number(process.env.TOPPS_BASEBALL_BULK_MAX_SOURCES || 100)),
);
const OUTPUT = resolve(
  process.cwd(),
  process.env.TOPPS_BASEBALL_BULK_OUTPUT ||
    ".checklist-discovery/topps-baseball-bulk-import-receipt.json",
);
const TEMP_ROOT = resolve(process.cwd(), ".checklist-discovery/topps-baseball-tmp");
const ALLOWED_STATUSES = (process.env.TOPPS_BASEBALL_BULK_STATUSES ||
...
function dbClient() {
  ...
  if (!url || !key) {
    throw new Error(
      "Topps baseball bulk import requires Supabase service-role access.",
    );
  }
...
function cleanFilename(sourceUrl: string, fallback: string) {
  try {
    ...
    // Use the release-name fallback below.
  }
...
async function download(sourceUrl: string) {
  const response = await fetch(sourceUrl, {
    headers: {
      Accept:
        "application/pdf,text/plain,text/csv,text/html,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,*/*",
      "Cache-Control": "no-cache",
      "User-Agent":
        "TCOS-Topps-Baseball-Bulk-Importer/1.0 (+private registry automation; contact sales@truelycollectables.com)",
    },
    redirect: "follow",
  ...
  if (bytes.byteLength > 50 * 1024 * 1024) {
    throw new Error(`Checklist exceeds the 50 MiB source limit (${bytes.byteLength} bytes).`);
  }
  return {
    bytes,
...
function extractText(params: {
  ...
  if (params.mimeType === "application/pdf") {
    ...
    try {
      return execFileSync("pdftotext", ["-layout", "-nopgbrk", filePath, "-"], {
        encoding: "utf8",
...
async function updateCatalog(
  ...
  if (error) {
    throw new Error(`Could not update checklist source catalog: ${error.message}`);
  }
}
...
async function main() {
  ...
  const query = db
    .from("checklist_source_catalog")
    .select(
      "source_url,source_sha256,release_name,status,metadata,adapter_id,adapter_version",
    )
  ...
  for (const row of rows || []) {
    ...
    try {
      ...
      if (text.length < 50) {
        throw new Error(`Checklist extraction produced only ${text.length} characters.`);
      }
      ...
      const common = {
        ...
        release_name: [
          validation.plan.release.releaseYear,
          validation.plan.release.product,
        ]
          .filter(Boolean)
          .join(" "),
      ...
      await updateCatalog(db, sourceUrl, {
        ...
        issue_summary: [
          {
            code: unsupported
              ? "topps_baseball_extractor_required"
              : "topps_baseball_bulk_import_failure",
            severity: "error",
  ...
  const receipt = {
    schema: "tcos.checklist.toppsBaseballBulkImportReceipt.v1",
    startedAt,
