...
export function titleFromHtml(html, fallback) {
  return decodeHtml(
    html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)?.[1]
      ...
      || fallback,
  ).replace(/\s*[-|]\s*(?:BaseballCardPedia|KeyMan Collectibles|Sports Card Radio|Cardboard Connection|Beckett).*$/i, "").trim();
}
...
export function extractMaker(title, categories = [], url = "") {
  ...
  for (const maker of MAKERS) {
    const escaped = escapeRegex(maker).replace(/\\ /g, "\\s+");
...
export function extractProduct(title, maker, sport) {
  let product = String(title)
    .replace(/\s*[-|]\s*(?:BaseballCardPedia|KeyMan Collectibles|Sports Card Radio|Cardboard Connection|Beckett).*$/i, "")
