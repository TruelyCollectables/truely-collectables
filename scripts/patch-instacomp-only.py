...
def remove_once_or_absent(path: str, old: str, marker: str, label: str) -> None:
    target = Path(path)
    text = target.read_text()
...
''',
    'provider: "openai_emergency"',
    "remove OpenAI emergency identity reader",
)
...
''',
    ...
    const primaryAiResult = await identifyCardWithConfiguredProviderFailover({
''',
    "InstaComp AI is the only identity engine",
    "remove emergency serial preflight",
)
...
''',
    "const serialOcr = null;",
    "disable external serial reader",
)
...
''',
    'requestedTier: "basic"',
    "disable external AI council",
)

Path("src/lib/instacomp-ai-provider-failover.ts").write_text(
...
Path("src/app/api/instacomp/internal-readiness/route.ts").write_text(
...
''',
    ...
    "defer perceptual-hash index until after migration",
)

Path("scripts/check-instacomp-provider-fallback.mjs").write_text(
...
if (!scan.includes("const serialOcr = null;")) {
...
Path("scripts/run-instacomp-internal-fail-closed-regressions.ts").write_text(
...
old_contract = '''const internalProvider = route.indexOf('provider: "instacomp_internal"');
...
'''
new_contract = '''const internalProvider = route.indexOf('provider: "instacomp_internal"');
...
'''
if new_contract not in contract:
    if old_contract not in contract:
        raise SystemExit("local-primary provider contract anchor missing")
    contract = contract.replace(old_contract, new_contract, 1)

old_readiness = '''requireText(
  readinessRoute,
  "checklistReady",
  "Production readiness must include the Checklist Registry.",
);
'''
new_readiness = '''requireText(
  readinessRoute,
  "checklistReady",
  "Production readiness must include the Checklist Registry.",
);
...
'''
if '"localModelReady"' not in contract:
    if old_readiness not in contract:
        raise SystemExit("local-primary readiness contract anchor missing")
    contract = contract.replace(old_readiness, new_readiness, 1)

old_log = '''console.log(
  "InstaComp internal memory -> Checklist Registry -> Ollama backup -> OpenAI emergency contract passed.",
);
'''
new_log = '''console.log("InstaComp AI-only identity contract passed.");
'''
if new_log not in contract:
    if old_log not in contract:
        raise SystemExit("local-primary success message anchor missing")
    contract = contract.replace(old_log, new_log, 1)

migration_check = '''const phashIndexInCreateScript = storage.indexOf(
  "CREATE INDEX IF NOT EXISTS scans_front_phash_idx ON scans(front_perceptual_hash);",
);
...
'''
if migration_check not in contract:
    marker = '''requireText(
  storage,
  "identity.serial_run",
...
'''
    if marker not in contract:
        raise SystemExit("storage migration contract anchor missing")
    contract = contract.replace(marker, marker + migration_check, 1)

contract_path.write_text(contract)

Path("services/instacomp-ai/tests/test_storage_migration.py").write_text(
    '''from __future__ import annotations
...
print("InstaComp-only patch applied")
