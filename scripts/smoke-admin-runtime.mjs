...
const smokeRoutes = [
  { path: "/admin/login", auth: false, expectedText: "Admin password" },
  { path: "/admin/reset-password", auth: false, expectedText: "Choose a permanent admin password" },
  { path: "/admin", auth: true, expectedText: "Command Center" },
  { path: "/admin/instacomp-direct", auth: true, expectedText: "InstaComp™ Direct Scan Lab" },
  { path: "/admin/instacomp/mobile", auth: true, expectedText: "InstaComp Mobile" },
  { path: "/admin/products", auth: true, expectedText: "Admin products" },
  { path: "/admin/products/new", auth: true, expectedText: "List Cards" },
  { path: "/admin/quick-list", auth: true, expectedText: "Accuracy Council + InstaComp™" },
  { path: "/admin/verified-reference-import", auth: true, expectedText: "Verified Reference → Pending Listings" },
  { path: "/admin/orders", auth: true, expectedText: "Orders" },
  { path: "/admin/order-notifications", auth: true, expectedText: "Order Notification Delivery" },
  { path: "/admin/sales-history", auth: true, expectedText: "Sold Collectibles" },
  { path: "/admin/offers", auth: true, expectedText: "Offers" },
  { path: "/admin/ebay/inventory-intake", auth: true, expectedText: "eBay Inventory Intake" },
  { path: "/admin/ebay/duplicates", auth: true, expectedText: "Duplicate cleanup queue" },
  { path: "/admin/financial-reconciliation", auth: true, expectedText: "Stripe Reconciliation" },
  { path: "/admin/market-intel", auth: true, expectedText: "Market Intel" },
  { path: "/admin/market-intel/kingmaker", auth: true, expectedText: "PROJECT KINGMAKER" },
  { path: "/admin/market-intel/kingmaker/capital-ledger", auth: true, expectedText: "Purchase Ledger Command" },
  { path: "/admin/production-smoke", auth: true, expectedText: "Production smoke" },
  { path: "/admin/live-payment-launch", auth: true, expectedText: "Live Payment" },
  { path: "/admin/live-shipping-launch", auth: true, expectedText: "Live Shipping" },
  { path: "/admin/settings", auth: true, expectedText: "Settings" },
  { path: "/admin/security", auth: true, expectedText: "Security" },
  { path: "/admin/accounts", auth: true, expectedTexts: ["Customer Account Lookup", "Customer Accounts"] },
  { path: "/admin/buyer-protection", auth: true, expectedText: "Buyer Protection Claims" },
  { path: "/admin/owner-seller-account", auth: true, expectedText: "Activate the owner seller account" },
  { path: "/admin/ebay", auth: true, expectedText: "eBay Reconciliation" },
  { path: "/admin/ebay/import-runner", auth: true, expectedText: "eBay Import Runner" },
  { path: "/admin/ebay/full-store-sync", auth: true, expectedText: "Full eBay Store Sync" },
  { path: "/admin/ebay/launch-ready-sync", auth: true, expectedText: "eBay Launch Readiness" },
  { path: "/admin/ebay/publish", auth: true, expectedText: "Pitch Black listing launcher" },
  { path: "/admin/ebay/sync-control", auth: true, expectedText: "eBay Sync Control" },
  { path: "/admin/files", auth: true, expectedText: "Admin Files" },
  { path: "/admin/instacomp", auth: true, expectedText: "InstaComp™ Scan Lab" },
  { path: "/admin/instacomp/v2", auth: true, expectedText: "InstaComp™ 2.0" },
  { path: "/admin/pending-card-import", auth: true, expectedText: "Card Intake & Listing" },
  { path: "/admin/instacomp/checklists", auth: true, expectedText: "Checklist Registry" },
  { path: "/admin/inventory", auth: true, expectedText: "Inventory Bridge" },
  { path: "/admin/inventory/category-review", auth: true, expectedText: "Import Category Review" },
  { path: "/admin/launch-gate-drill", auth: true, expectedText: "Launch Gate Drill" },
  { path: "/admin/launch-readiness", auth: true, expectedText: "Launch Readiness" },
  { path: "/admin/market-intel/readiness", auth: true, expectedText: "Readiness Control Board" },
  { path: "/admin/market-intel/watchlist", auth: true, expectedText: "Player Watchlist" },
  { path: "/admin/market-intel/watch-center", auth: true, expectedText: "Who, What, and When to Investigate" },
  { path: "/admin/market-intel/comps", auth: true, expectedText: "Exact-Card Sold Comps" },
  { path: "/admin/market-intel/discovery", auth: true, expectedText: "Licensed-Card Discovery Desk" },
  { path: "/admin/market-intel/ebay", auth: true, expectedText: "eBay Active Listing Scanner" },
  { path: "/admin/market-intel/deals", auth: true, expectedText: "Shark List™ Deal Engine" },
  { path: "/admin/market-intel/growth-specs", auth: true, expectedText: "Growth Spec Lab™" },
  { path: "/admin/market-intel/growth-specs/prospects", auth: true, expectedText: "Licensed Pro Value Watchlists" },
  { path: "/admin/market-intel/buy", auth: true, expectedText: "Buy + Track Desk" },
  { path: "/admin/market-intel/portfolio", auth: true, expectedText: "Portfolio Intelligence" },
  { path: "/admin/market-intel/purchases", auth: true, expectedText: "Purchase Ledger" },
  { path: "/admin/market-intel/purchases/deleted", auth: true, expectedText: "Duplicate removed" },
  { path: "/admin/market-intel/purchases/new", auth: true, expectedText: "Card Show + Card Shop Purchase" },
  { path: "/admin/market-intel/purchases/ebay-intake", auth: true, expectedText: "eBay Purchase Inbox" },
  { path: "/admin/market-intel/ingestion", auth: true, expectedText: "Ingestion Health" },
  { path: "/admin/market-intel/reports", auth: true, expectedText: "Intelligence Report Desk" },
  { path: "/admin/market-intel/delivery", auth: true, expectedText: "Email Delivery Center" },
  { path: "/admin/market-intel/delivery/test", auth: true, expectedText: "Send a controlled test email" },
  { path: "/admin/order-review-cases", auth: true, expectedText: "Order Review Case Queue" },
  { path: "/admin/payment-simulations", auth: true, expectedText: "Payment Simulation Lab" },
  { path: "/admin/seller-payouts", auth: true, expectedText: "Seller Payout Review" },
  { path: "/admin/shipping", auth: true, expectedText: "Label + Coverage Control" },
  { path: "/admin/shipping/simulations", auth: true, expectedText: "Shipping Simulation Lab" },
];
...
async function smokeRoute(route, cookieHeader) {
  ...
  if (expectedTexts.length > 0 && !expectedTexts.some((text) => body.includes(text))) {
  ...
  return { ...route, status: response.status, ok: failures.length === 0, failures };
}
...
async function smokeFirstProductDetail(cookieHeader) {
  ...
  if (response.status !== 200) {
    return { path: "/admin/products/[first]", status: response.status, ok: false, failures: ["could not load product list for detail-route discovery"] };
  }
  ...
  return smokeRoute({ path, auth: true, expectedText: "Product command desk" }, cookieHeader);
}
...
async function smokeFirstOrderDetail(cookieHeader) {
  ...
  const body = await response.text().catch(() => "");
  if (response.status !== 200) {
    return { path: "/admin/orders/[first]", status: response.status, ok: false, failures: ["could not load order list for detail-route discovery"] };
  }
  const orderDetailMatch = body.match(/href="(\/admin\/orders\/\d+(?:\?[^"]*)?)"/);
  if (!orderDetailMatch) return null;
  const path = orderDetailMatch[1].replaceAll("&amp;", "&");
  return smokeRoute({ path, auth: true, expectedText: "Order command desk" }, cookieHeader);
}
...
try {
  ...
  for (const route of smokeRoutes) results.push(await smokeRoute(route, cookieHeader));
