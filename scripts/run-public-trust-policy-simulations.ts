...
assert.equal(BUYER_PROTECTION_PATH, "/buyer-protection");
...
assert.ok(
  layout.includes('import Footer from "./components/Footer"'),
  "Root layout must import the footer.",
);
assert.ok(layout.includes("<Footer />"), "Root layout must render the footer on every page.");
assert.ok(
  layout.includes("email: STORE_SUPPORT_EMAIL"),
  "Organization metadata must expose the support email.",
);
...
assert.ok(
  cartPage.includes("CheckoutPolicyNotice"),
  "Cart must render the policy notice before payment.",
);
...
for (const token of [
  ...
  assert.ok(
    ...
    `Shipping policy must use checkout constant ${token}.`,
  );
}
assert.ok(
...
assert.ok(
  privacy.includes("Payments are processed by Stripe"),
  "Privacy policy must explain payment processing.",
);
assert.ok(
  privacy.includes("do not sell customer personal information"),
  "Privacy policy must state the advertising-data position.",
);
assert.ok(
  privacy.includes("STORE_SUPPORT_EMAIL"),
  "Privacy policy must provide a contact method.",
);

const returnsPolicy = read("src/app/returns/page.tsx");
assert.ok(
  returnsPolicy.includes("Do not mail an item back"),
  "Returns must require authorization before shipment.",
);
assert.ok(
  returnsPolicy.includes("original payment method"),
  "Returns must explain the refund destination.",
);
assert.ok(
  returnsPolicy.includes("STORE_SUPPORT_EMAIL"),
  "Returns must provide a support contact.",
);

const contact = read("src/app/contact/page.tsx");
assert.ok(
  contact.includes("STORE_SUPPORT_EMAIL"),
  "Contact page must use the central support email.",
);
assert.ok(
  contact.includes("Do not email full payment-card numbers"),
  "Contact page must warn customers not to send sensitive credentials.",
);
