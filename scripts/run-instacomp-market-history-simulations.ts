...
assert.equal(first.observation_fingerprint, duplicate.observation_fingerprint, "Repeated discovery of the same sold comp must dedupe even when observed later.");
...
assert.equal(trend.asksUsedAsSoldValue, false, "An inflated asking price must never become sold-market value.");

const insufficient = calculateExactCardMarketTrend([
  { observation_kind: "ASK", delivered_price: 100, effective_at: null, observed_at: "2026-08-01T00:00:00Z" },
]);
