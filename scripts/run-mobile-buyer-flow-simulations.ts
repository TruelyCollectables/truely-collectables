...
assert.match(shop, /className="object-contain p-3"/, "Shop thumbnails must show the whole card instead of cropping it.");
...
assert.match(productGallery, /grid grid-cols-4 gap-2 sm:grid-cols-6/, "Product thumbnails must remain usable on narrow phones and scale up at larger breakpoints.");
assert.match(productGallery, /aspect-\[3\/4\]/, "Product images must preserve a stable card-shaped mobile aspect ratio.");
assert.match(productGallery, /object-contain/, "Product gallery images must show the whole card instead of cropping it.");
