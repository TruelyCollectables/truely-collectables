...
const BUNDLE_SUFFIX = ".pokemon-ja-incomplete-reconciled.bundle.json";
...
type ImportReceiptRow = {
  ...
  preservedNamedCards: number | null;
  nameBackfilledCards: number | null;
...
function usage() {
  console.error(
    [
      "Usage:",
      "  npx tsx scripts/import-pokemon-japanese-incomplete-reconciled-bundles.ts <bundle-file-or-directory> [--apply] [--receipt <path>] [--continue-on-error]",
      "",
      "Default behavior validates only. --apply requires Supabase service-role environment variables and creates the first complete active version for each approved set.",
    ].join("\n"),
  );
}
...
function readHeader(content: Buffer): BundleHeader {
  ...
  if (parsed.schema !== POKEMON_JAPANESE_INCOMPLETE_RECONCILED_SCHEMA) {
    throw new Error(
      `Unsupported incomplete reconciliation schema: ${String(parsed.schema)}.`,
    );
  }
  return parsed;
}
...
async function main() {
  ...
  const receiptPath = resolve(
    argumentValue("--receipt") ||
      `.codex-run/pokemon-ja-incomplete-reconciled-${apply ? "import" : "validation"}-receipt.json`,
  );
  ...
  for (const file of files) {
    ...
    let preservedNamedCards: number | null = null;
    let nameBackfilledCards: number | null = null;
    ...
    try {
      ...
      expectedOfficialCards = integer(header.official?.comparableCardCount);
      sourceCards = integer(header.official?.sourceCardCount);
      preservedNamedCards = integer(
        header.official?.preservedNamedCardCount,
      );
      nameBackfilledCards = integer(
        header.official?.nameBackfilledCardCount,
      );
      addedOfficialCards = integer(header.official?.addedCardCount);
  ...
  const totals = rows.reduce(
    (sum, row) => {
      ...
      sum.preservedNamedCards += row.preservedNamedCards || 0;
      sum.nameBackfilledCards += row.nameBackfilledCards || 0;
  ...
  const receipt = {
    schema:
      "tcos.checklist.pokemonJapaneseIncompleteReconciledImportReceipt.v1",
