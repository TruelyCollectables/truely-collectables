...
async function main() {
  ...
  if (fp.auditedSets !== 5643 || fp.archiveBearingSetKeys !== 5268 || fp.sourceRunId !== "31100986894") {
    throw new Error(`Reconciliation corpus lost frozen provenance: ${JSON.stringify(fp)}.`);
  }
  if ((manifest.sets || []).length !== 5549) throw new Error(`Reconciliation expected 5,549 sanitized records, found ${manifest.sets?.length || 0}.`);
  if (Number(manifest.counts?.import || 0) !== 5094 || Number(manifest.counts?.alias_of || 0) !== 62) {
    throw new Error(`Reconciliation canonical/alias fingerprint changed: ${manifest.counts?.import || 0}/${manifest.counts?.alias_of || 0}.`);
  }
  ...
  for (const set of importSets) {
    const matches = (set.candidates || [])
      ...
      .filter(({ row }) =>
        row &&
        ["imported", "unchanged"].includes(row.status) &&
        row.metadata?.masterArchiveRunId === "31100986894" &&
        row.metadata?.masterArchiveExactSetKey === set.exactSetKey
      );
    ...
    if (!matches.length) {
      unresolved.push({
        ...
        aliases: set.aliasExactSetKeys || [],
        candidates: (set.candidates || []).slice(0, 8).map((candidate) => {
          const row = byUrl.get(candidate.sourceUrl);
          return {
            source: candidate.source,
            sourceUrl: candidate.sourceUrl,
            sourceChecklistRows: candidate.sourceChecklistRows ?? null,
            validationChecklistRows: candidate.validationChecklistRows ?? candidate.checklistRows ?? null,
            status: row?.status || "missing_catalog_row",
            catalogExactSetKey: row?.metadata?.masterArchiveExactSetKey || null,
            catalogMasterRunId: row?.metadata?.masterArchiveRunId || null,
    ...
    const best = matches.sort(
      (a, b) => Number(b.row.validation_counts?.cards || 0) - Number(a.row.validation_counts?.cards || 0),
    )[0];
  ...
  const receipt = {
    schema: "tcos.checklist.masterArchiveReconciliation.v4",
    checkedAt: new Date().toISOString(),
    frozenSourceRunId: fp.sourceRunId,
    originalAuditedSetKeys: fp.auditedSets,
    originalArchiveBearingSetKeys: fp.archiveBearingSetKeys,
    sanitizedSetRecords: manifest.sets.length,
    dispositionCounts: manifest.counts,
    expectedCanonicalImportSets: importSets.length,
    promotedCanonicalImportSets: covered.length,
    remainingCanonicalImportSets: unresolved.length,
    resolvedAliasSets: Number(manifest.counts?.alias_of || 0),
    promotedTotals: { cards, identities },
    exactSanitizedProvenanceRequired: true,
    complete: covered.length === importSets.length && unresolved.length === 0,
