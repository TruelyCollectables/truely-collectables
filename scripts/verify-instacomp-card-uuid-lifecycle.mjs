...
const soldHistory = read('src/lib/collectible-sale-history.ts');
...
const required = [
  ...
  [soldHistory, 'metadataCardUuid(inventory?.metadata)', 'sold-history metadata UUID fallback'],
  [soldHistory, 'cardUuid,', 'sold-history universal UUID retention'],
...
console.log('PASS one permanent physical card UUID survives inventory → product → sold history → universal inventory → order item and remains searchable');
