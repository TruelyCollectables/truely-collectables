...
function replaceExact(path, before, after) {
  ...
  if (!source.includes(before)) throw new Error(`Expected Stripe fragment missing in ${path}.`);
  const next = source.replace(before, after);
  if (next === source) throw new Error(`Stripe hardening made no change in ${path}.`);
...
console.log("Applied dedicated Stripe live credential hardening on latest main.");
