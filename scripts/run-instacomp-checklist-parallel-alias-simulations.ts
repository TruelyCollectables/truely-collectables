...
function decide(parallel: string | null, candidates: InstaCompChecklistCandidate[] = [baseCandidate]) {
...
for (const alias of aliases) {
  ...
  assert.equal(result.status, "exact_match", `${alias} should resolve to the canonical Prizms Ice identity`);
  assert.equal(result.match?.identityId, "ice-id");
  console.log(`PASS ${alias} -> Prizms Ice`);
}

for (const unsafeAlias of ["Cracked Ice", "White Ice", "Blue Prizm", "Silver Prizm"]) {
  const result = decide(unsafeAlias);
  assert.equal(result.status, "review_required", `${unsafeAlias} must not be coerced into Prizms Ice`);
...
assert.equal(missingParallel.status, "review_required", "Missing parallel evidence must not choose between Base and Ice");
