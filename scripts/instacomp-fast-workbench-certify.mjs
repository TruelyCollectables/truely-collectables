...
const checks = [
  ...
  ['workbench shows complete seller audit packet', scanner.includes('Seller:') && scanner.includes('sellerFeedbackPercent') && scanner.includes('Item #') && scanner.includes('Shipping') && scanner.includes('Open listing')],
  ...
  ['Sentinel admin fetches fail safely', sentinel.includes('async function readProxy') && sentinel.includes('response.json().catch(() => ({}))') && sentinel.includes('readProxy(statusResponse') && sentinel.includes('readProxy(response, "Sentinel action failed.")')],
  ['Sentinel dashboard uses certified proxy', sentinel.includes('/api/instacomp/checklist-sentinel?view=status') && sentinel.includes('fetch("/api/instacomp/checklist-sentinel"') && !sentinel.includes('/api/admin/instacomp/checklist-sentinel')],
