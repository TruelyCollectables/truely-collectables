...
cron = replaceOrThrow(
  ...
  "eBay cron error collection",
);

const cronStateBefore = [
  "  let activeLinkedProducts: number | null = null;",
  "",
  "  try {",
  "    firstAuthoritative",
].join("\n");
const cronStateAfter = [
  "  let activeLinkedProducts: number | null = null;",
...
cron = replaceOrThrow(
  ...
  cronStateBefore,
  cronStateAfter,
  "eBay cron initial authoritative stage",
);

const authoritativeCatchBefore = [
  "  } catch (error) {",
  "    errors.push({",
...
cron = replaceOrThrow(
  cron,
  authoritativeCatchBefore,
  authoritativeCatchAfter,
  "eBay cron authoritative catch",
);
cron = replaceOrThrow(
  cron,
...
notifications = replaceOrThrow(
  ...
  "notification retry backoff query",
);
...
console.log(
  "Applied durable post-sale eBay quantity and notification retry safety changes.",
);
