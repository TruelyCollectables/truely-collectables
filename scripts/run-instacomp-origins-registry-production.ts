...
assert(plan.validation.status === "passed", "Origins checklist slice did not validate.");
assert(plan.validation.counts.sets === 1, "Expected one Base - Rookies cardset.");
assert(plan.validation.counts.cards === 50, "Expected all 50 Base - Rookies cards.");
assert(plan.validation.counts.parallels === 19, "Expected all 19 listed rookie parallels.");
assert(plan.validation.counts.identities === 1000, "Expected 1,000 exact rookie identities.");

const shedeurCard = plan.cards.find(
  (card) => card.cardNumber === "107" && card.players.includes("Shedeur Sanders"),
);
assert(shedeurCard, "Shedeur Sanders #107 is missing from the normalized plan.");
...
assert(shedeurHoloBlue, "Shedeur Sanders #107 Holo Blue /199 identity is missing.");
...
const persistence = await importChecklistArtifact({ artifact, validateOnly: false });
assert(persistence.ok && !persistence.validatedOnly, "Production Registry import did not persist.");
...
assert(resolution.match.player === "Shedeur Sanders", "Production player mismatch.");
assert(resolution.match.cardNumber === "107", "Production card-number mismatch.");
assert(resolution.match.parallel === "Holo Blue", "Production parallel mismatch.");
...
assert(identityDecision.confidence >= 0.95, "Production identity confidence is below 95%.");
