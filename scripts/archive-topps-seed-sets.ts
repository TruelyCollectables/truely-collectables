...
async function download(seed: Seed) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      if (!trusted(seed.url)) throw new Error(`Untrusted Topps asset host: ${seed.url}`);
      const response = await fetch(seed.url, { headers: { "User-Agent": "TCOS-Topps-Archive/4.0", Accept: "application/pdf,*/*" }, redirect: "follow", signal: AbortSignal.timeout(90_000) });
      ...
      if (Buffer.from(bytes.subarray(0, 5)).toString("ascii") !== "%PDF-") throw new Error("PDF signature missing");
...
async function main() {
  ...
  for (const seed of seeds) {
    try {
      ...
      files.push({ manufacturer: "Topps", ...seed, finalUrl: result.finalUrl, archivePath: relativePath, filename, sha256, sizeBytes: result.bytes.length, mimeType: "application/pdf", attempts: result.attempt });
    } catch (error) {
