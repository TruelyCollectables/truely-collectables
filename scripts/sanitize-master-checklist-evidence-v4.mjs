...
const GENERIC_SPORT_WORD = {
  baseball: new Set(["baseball"]),
  basketball: new Set(["basketball"]),
  football: new Set(["football"]),
  hockey: new Set(["hockey"]),
  soccer: new Set(["soccer"]),
  mma: new Set(["mma"]),
  wrestling: new Set(["wrestling"]),
  racing: new Set(["racing"]),
  golf: new Set(["golf"]),
  tennis: new Set(["tennis"]),
  boxing: new Set(["boxing"]),
  "multi-sport": new Set(["multisport", "multi", "sport", "multiple", "sports"]),
};
...
function aliasCore(set) {
  const noise = new Set([
    ...GENERIC_ALIAS_NOISE,
    ...(GENERIC_SPORT_WORD[String(set.sport || "").toLowerCase()] || []),
  ]);
...
function canonicalSort(a, b) {
  const left = canonicalScore(a);
  const right = canonicalScore(b);
...
function dedupeCandidates(candidates) {
  ...
  for (const candidate of candidates) {
    ...
    if (!prior || Number(candidate.checklistRows || 0) > Number(prior.checklistRows || 0)) {
...
if (fp.auditedSets !== 5643 || fp.archiveBearingSetKeys !== 5268 || fp.sourceRunId !== "31100986894") {
  throw new Error(`Normalized corpus lost original frozen fingerprint: ${JSON.stringify(fp)}.`);
}
if (!manifest.sportNormalization?.ok || manifest.sportNormalization.unresolvedSourceCount !== 0) {
  throw new Error("Sport normalization must pass with zero unresolved sources before evidence sanitization.");
}
if ((manifest.sets || []).length !== 5549) throw new Error(`Sport-normalized corpus expected 5,549 sets, found ${manifest.sets?.length || 0}.`);
const beforeCounts = manifest.counts || {};
for (const [key, expected] of Object.entries({ import: 5156, gap_pending: 331, excluded_non_sport: 53, aggregate_index: 8, needs_name_review: 1 })) {
  if (Number(beforeCounts[key] || 0) !== expected) throw new Error(`Sport-normalized ${key} count changed: expected ${expected}, found ${beforeCounts[key] || 0}.`);
}
  ...
  for (const candidate of set.candidates || []) {
    ...
    for (const file of evidenceFiles(candidate)) {
      ...
      if (file.duplicateOf) {
        ...
        if (set.exactSetKey === "baseball|2021|topps|inception" && /opening-day/i.test(String(file.duplicateOf))) {
...
if (unsafeOriginalCollisions.length) {
  writeJson(auditOutput, {
    schema: "tcos.checklist.masterArchiveEvidenceAudit.v4",
    checkedAt: new Date().toISOString(),
    ok: false,
    reason: "cross_set_sha_has_multiple_direct_originals",
    unsafeOriginalCollisions,
  });
  throw new Error(`Found ${unsafeOriginalCollisions.length} cross-release evidence SHAs with multiple direct originals.`);
}
if (!knownContamination.length) {
  throw new Error("Regression guard did not find the known 2021 Topps Inception -> Opening Day inherited evidence.");
}

let blockedDuplicateEvidenceFiles = 0;
...
for (const set of manifest.sets) {
  ...
  for (const candidate of set.candidates || []) {
    ...
    affectedSets.add(set.exactSetKey);
    blockedDuplicateEvidenceFiles += blocked.length;
...
// Alias only deterministic non-identity noise. League/promotion names are
// deliberately NOT noise: NBA != WNBA, WWE != AEW, UFC remains identity-bearing,
// and NFL/NHL/MLB are preserved when explicitly present.
...
for (const [aliasKey, members] of aliasBuckets) {
  ...
  const ordered = [...members].sort(canonicalSort);
  ...
  for (const member of ordered) {
    for (const candidate of member.candidates || []) {
      mergedCandidates.push({ ...candidate, aliasSourceExactSetKey: member.exactSetKey });
    }
  }
...
manifest.evidenceSanitization = {
  schema: "tcos.checklist.masterArchiveEvidenceSanitization.v4",
  duplicateReferencesBlocked: blockedDuplicateEvidenceFiles,
  deterministicAliasesResolved: aliasResolvedCount,
  aliasPolicy: "generic wording + generic sport noun only; league/promotion tokens preserved",
};
...
const leftovers = manifest.sets.flatMap((set) =>
  (set.candidates || []).flatMap((candidate) => (candidate.files || []).filter((file) => file.duplicateOf)),
);
if (leftovers.length) throw new Error(`Sanitized corpus still contains ${leftovers.length} inherited evidence references.`);
for (const alias of manifest.sets.filter((set) => set.disposition === "alias_of")) {
  if (!alias.aliasOfExactSetKey || (alias.candidates || []).length) throw new Error(`Alias ${alias.exactSetKey} was not fully collapsed.`);
}
...
const audit = {
  ...
  checkedAt: new Date().toISOString(),
  ok: true,
  originalCorpusFingerprint: fp,
  sportNormalizedSetCount: manifest.sets.length,
  sportNormalizedImportSetsBeforeAlias: Number(beforeCounts.import || 0),
  candidateCount,
  evidenceFileCount,
  uniqueEvidenceShaCount: shaEvidence.size,
  duplicateReferenceFiles,
  blockedDuplicateEvidenceFiles,
  affectedCandidates,
  affectedSets: affectedSets.size,
  crossSetCollisionCount: crossSetCollisions.length,
  unsafeOriginalCollisionCount: 0,
