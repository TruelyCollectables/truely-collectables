...
DEFAULT_DEAL_HUNTER_LESSONS = (
    ...
    DealHunterLesson("unsigned_auth_optional_official_ball_required", "signed_baseballs", "Prefer promising unauthenticated prospect autographs when the listing photos verify that the signature is on an official Major League Baseball or official Minor League Baseball. Authentication or a COA is optional; generic, collegiate, practice, souvenir, photo and bat items do not qualify.", "The operator accepts unauthenticated signatures but requires the physical ball itself to be official MLB or MiLB merchandise."),
    DealHunterLesson("music_comedy_direct_unsigned_flat_items", "music_comedy_autographs", "Prefer cheap unauthenticated country, metal, rock, classic-rock and comedian autographs when the item is a poster, 11x17/11x18 show poster, CD cover, album cover, vinyl/LP jacket, booklet, insert or setlist and the listing suggests artist-direct, official-store, merch-table, VIP, meet-and-greet, venue or show purchase provenance. Authentication is optional; reject premium-priced listings by default and exclude facsimile/preprint signatures, unrelated merch, shirts, hats, picks, drumsticks, cards and cut signatures.", "The operator wants raw artist/band/comedian signed flat items only when the price leaves real upside, not fully priced authenticated memorabilia."),
