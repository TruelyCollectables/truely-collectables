...
from .models import AnalyzeResponse, CardIdentity, HealthResponse, LearningState, LessonCreate, LessonRecord
...
app = FastAPI(
    ...
    description="Private local evidence reader with central Checklist Registry identity locking.",
)
...
@app.post("/v1/scans/analyze", response_model=AnalyzeResponse, dependencies=[Depends(require_api_key)])
async def analyze_scan(front: UploadFile = File(...), back: UploadFile | None = File(default=None)) -> AnalyzeResponse:
    ...
    try:
        ...
        back_image = validate_and_normalize_image(back_content, settings.max_image_bytes) if back_content else None
    ...
    model_error = None
    try:
        suggestion = await reader.analyze(front_image.content, back_image.content if back_image else None)
    ...
    if checklist_result.outcome.value == "exact_match" and checklist_result.identity and checklist_result.identity_id:
        trusted_identity = checklist_result.identity
        pricing_allowed = True
        learning_allowed = True
        status = "trusted_memory_match"
        next_action = "Exact Registry identity locked. Continue to verified marketplace comps."
    elif model_error:
        ...
        next_action = "Connect the Mac service to the authenticated central Checklist Registry."
    else:
        status = "needs_review"
        next_action = "Review evidence or provide clearer front and back images; pricing remains blocked."
    ...
    if pricing_allowed and trusted_identity:
        store.create_lesson(LessonCreate(
            scan_id=scan_id,
            ...
            notes="Automatically promoted only after exact central Registry lock.",
        ))
    return result


@app.post("/v1/lessons", response_model=LessonRecord, dependencies=[Depends(require_api_key)])
async def create_lesson(request: LessonCreate) -> LessonRecord:
    if request.state in {LearningState.OBSERVED, LearningState.CHECKLIST_CONFIRMED}:
        raise HTTPException(status_code=409, detail="This learning state may only be created by the scan pipeline.")
    if request.state == LearningState.OPERATOR_CONFIRMED and not request.operator_id:
        raise HTTPException(status_code=400, detail="operator_id is required for operator-confirmed lessons.")
    try:
...
@app.get("/v1/lessons/search", dependencies=[Depends(require_api_key)])
async def search_lessons(player: str | None = Query(default=None), year: str | None = Query(default=None),
                         set_name: str | None = Query(default=None), card_number: str | None = Query(default=None),
                         parallel: str | None = Query(default=None), limit: int = Query(default=10, ge=1, le=50)):
    identity = CardIdentity(player=player, year=year, set_name=set_name, card_number=card_number, parallel=parallel)
    if not any(identity.model_dump().values()):
        raise HTTPException(status_code=400, detail="At least one identity field is required")
    return {"schema_version": "tcos.instacomp-ai.memory-search.v1",
