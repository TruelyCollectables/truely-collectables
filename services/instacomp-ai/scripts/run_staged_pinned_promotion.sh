...
target="$service_root/scripts/promote_lora_candidate_frozen_25_v19.py"
...
[[ -f "$target" ]] || {
  echo "V19 staged promotion runner is missing: $target" >&2
  exit 2
}
...
if [[ -n "${INSTACOMP_AI_REGISTRY_TOKEN:-}" || -n "${INSTACOMP_AI_SENTINEL_ARCHIVE_TOKEN:-}" ]]; then
  ...
  echo "Refusing V19 staged promotion: Registry authentication is missing after loading $service_root/.env." >&2
...
if [[ "$self_test" == "0" ]]; then
  echo "INFO Hydrating deterministic visual-pattern evidence before V19 preflight"
  "$service_python" "$visual_memory_repair" \
    --source-contains supervised_203_operator_confirmed \
    --max-repairs 250 \
    --workers 6

  echo "INFO Hydrating reviewed pinned visual evidence; V19 still requires fresh Registry and physical admission"
  "$service_python" "$pinned_visual_memory_repair" "$@"
fi

echo "PASS pinned staged promotion launcher mode: promotion-v19-single-canonical-pipeline"
echo "INFO V19 does not install V18/V15/V14 runtime monkey-patch contracts"
echo "INFO V19 preflight, candidate qualification, and both final rounds share one Registry resolver and one physical gate"
echo "INFO V19 candidate qualification calls the validated LoRA sidecar directly; Ollama fallback is forbidden during promotion"
...
echo "INFO Historical row-local Registry receipts are advisory only for fresh selection"
