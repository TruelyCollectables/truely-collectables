...
export async function GET(
  ...
  try {
    ...
    const postage = metadataRecord(
      (data as ShippingLabelRow).metadata,
    ...
    if (postage?.status !== "purchased" || !providerPdfUrl) {
      return Response.json(
        { error: "No paid ShipStation parcel PDF is available." },
        { status: 404 },
      );
    }
    ...
    return new Response(response.body, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="tcos-order-${orderId}-usps-parcel.pdf"`,
    ...
    return Response.json(
      { error: error?.message || "Could not load paid parcel PDF." },
      { status: 500 },
    );
  }
}
