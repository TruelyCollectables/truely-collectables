...
export async function POST(request: Request) {
  try {
    ...
    if (!/^https:\/\//i.test(localUrl) || !localKey) {
      return response({
        ok: false,
        code: "LOCAL_RUNTIME_CONFIG_UNAVAILABLE",
        has_local_url: Boolean(localUrl),
    ...
    for (const [side, file] of [["front", front], ["back", back]] as const) {
      if (!ALLOWED_IMAGE_TYPES.has(file.type) || file.size <= 0 || file.size > MAX_IMAGE_BYTES) {
        return response({ ok: false, code: "DIAGNOSTIC_IMAGE_REJECTED", side }, 400);
      }
    }
    ...
    const [healthRes, doctorRes] = await Promise.all([
      ...
      fetch(`${localUrl}/v1/control/doctor`, {
        headers: authHeaders,
        cache: "no-store",
    ...
    const scanRes = await fetch(`${localUrl}/v1/scans/analyze`, {
      method: "POST",
      headers: authHeaders,
      body: macForm,
      cache: "no-store",
    ...
    return response({
      ...
      health: {
        ...
        http_ok: healthRes.ok,
        body: healthBody,
      },
      system_doctor: {
        http_status: doctorRes.status,
        http_ok: doctorRes.ok,
        body: doctorBody,
      },
      scan: {
        http_status: scanRes.status,
        http_ok: scanRes.ok,
    ...
    return response({
      ok: false,
      code: "RAW_MAC_DIAGNOSTIC_FAILED",
