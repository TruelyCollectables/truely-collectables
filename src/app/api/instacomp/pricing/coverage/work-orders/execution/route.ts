...
function batchFailure(error: unknown, index: number, attackKey: unknown) {
  return {
    ...
    code: error instanceof InstaCompJobServerError ? error.code : "KINGMAKER_EXECUTION_ITEM_FAILED",
    error: error instanceof Error ? error.message : "Work-order execution failed.",
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (Array.isArray(body.items)) {
      if (body.items.length < 1) return NextResponse.json({ ok: false, error: "At least one work order is required.", code: "KINGMAKER_EXECUTION_BATCH_EMPTY" }, { status: 400, headers: { "cache-control": "no-store" } });
      if (body.items.length > MAX_BATCH_SIZE) return NextResponse.json({ ok: false, error: `A maximum of ${MAX_BATCH_SIZE} work orders may be updated at once.`, code: "KINGMAKER_EXECUTION_BATCH_TOO_LARGE" }, { status: 400, headers: { "cache-control": "no-store" } });

      const workOrders: unknown[] = [];
      const failures: ReturnType<typeof batchFailure>[] = [];
      for (const [index, item] of body.items.entries()) {
        if (!item || typeof item !== "object" || Array.isArray(item)) {
          failures.push(batchFailure(new Error("Each batch item must be a JSON object."), index, null));
          continue;
        }
      ...
      }, { status: failures.length === body.items.length ? 409 : 200, headers: { "cache-control": "no-store" } });
    }
