...
function gcsRawUrl(name: string) { return `https://storage.googleapis.com/${GCS_BUCKET}/${name}`; }
function gcsFetchUrl(name: string) { return `https://storage.googleapis.com/${GCS_BUCKET}/${name.split("/").map(encodeURIComponent).join("/")}`; }
...
async function readLiveInventory(supabase: ReturnType<typeof createSupabaseServerClient>, storeId: string) {
  ...
  for (let page = 0; page < 20; page += 1) {
    ...
    if (error) throw error;
    const batch = data || []; existingRows.push(...batch); if (batch.length < PAGE_SIZE) break;
  }
...
async function classify(rows: CsvRow[], supabase: ReturnType<typeof createSupabaseServerClient>, storeId: string): Promise<Classification> {
  ...
  const assign = (collxId: string, productId: number, reason: string, score: number) => {
    ...
    const row = sourceById.get(collxId); const need = row ? qty(row) : 1; const capacity = remainingCapacity.get(productId) || 0;
    if (!productById.has(productId) || capacity < need) return false;
    assigned.add(collxId); remainingCapacity.set(productId, capacity - need);
    matches.push({ collx_id: collxId, product_id: productId, reason, score }); return true;
  };
  const hashToSource = new Map<string, string>();
  for (const row of rows) for (const side of [images[row.collx_id]?.front, images[row.collx_id]?.back]) if (side) hashToSource.set(sha256(gcsRawUrl(side)), row.collx_id);
  for (const product of products) {
    const found = new Set<string>();
    for (const url of [product.image_url || "", ...(product.inventory_images || [])]) {
      const value = String(url || ""); const mirror = value.match(/\/collx-mirror\/([0-9a-f]{64})\./i); const raw = value.match(/collx-product-images\/(\d+)-[12]-/i);
  ...
  for (const row of rows) {
    ...
    for (const product of products) {
      ...
      const hit = candidate(row, product); if (hit) candidates.push(hit);
    }
  }
  const bySource = new Map<string, Candidate[]>(); const byProduct = new Map<number, Candidate[]>();
  ...
  for (const [collxId, list] of bySource) {
    const top = list[0]; if (!top || top.score < 110) continue; const second = list[1]?.score ?? -999;
    const evidence = [top.year, top.number, top.serial, top.price].filter(Boolean).length;
    if (top.score - second < 15 || evidence < 3) continue;
    const productList = byProduct.get(top.product_id) || []; if (productList[0]?.collx_id !== collxId) continue;
    const productSecond = productList[1]?.score ?? -999; if (top.score - productSecond >= 15) reciprocal.push(top);
  }
  reciprocal.sort((a,b) => b.score - a.score); for (const hit of reciprocal) assign(hit.collx_id, hit.product_id, "reciprocal_exact_identity", hit.score);
  const strong = candidates.filter((hit) => hit.score >= 105 && hit.year && hit.number && (hit.serial || hit.price || hit.set_hits >= 2))
  ...
  const summary = {
    sourceRows: rows.length, ebayProducts: products.length, existingCollxImports: live.existing.size,
    duplicateCollxRows: assigned.size, importCandidateRows: importIds.length,
    ...
    sourceRowsWithBack: rows.filter((row) => Boolean(images[row.collx_id]?.back)).length, databaseWrites: false,
  };
...
async function saveClassification(supabase: ReturnType<typeof createSupabaseServerClient>, classification: Classification) {
  const { error } = await supabase.storage.from(OWNED_BUCKET).upload(CLASSIFICATION_PATH, Buffer.from(JSON.stringify(classification)), { contentType: "application/json", upsert: true });
  if (error) throw error;
}
...
function imageExtension(name: string) { const match = name.toLowerCase().match(/\.(jpe?g|png|webp)(?:$|\?)/); return match ? (match[1] === "jpeg" ? "jpg" : match[1]) : "jpg"; }
async function copyImage(supabase: ReturnType<typeof createSupabaseServerClient>, collxId: string, side: "front" | "back", sourceName?: string) {
  ...
  const response = await fetch(gcsFetchUrl(sourceName), { cache: "no-store" }); if (!response.ok) throw new Error(`image ${sourceName} ${response.status}`);
  const bytes = Buffer.from(await response.arrayBuffer()); const ext = imageExtension(sourceName); const path = `collx-full/20260811/${collxId}/${side}.${ext}`;
  const contentType = response.headers.get("content-type") || (ext === "png" ? "image/png" : ext === "webp" ? "image/webp" : "image/jpeg");
  const { error } = await supabase.storage.from(OWNED_BUCKET).upload(path, bytes, { contentType, upsert: true }); if (error) throw error;
  return supabase.storage.from(OWNED_BUCKET).getPublicUrl(path).data.publicUrl;
}

async function ensureImportedRow(supabase: ReturnType<typeof createSupabaseServerClient>, storeId: string, row: CsvRow, imageEntry: ImageEntry) {
  const sku = `COLLX-${row.collx_id}`; const title = titleFor(row); const asking = Math.max(0, num(row.asking_price) ?? 0); const quantity = qty(row);
  const imageErrors: string[] = []; let frontUrl: string | null = null; let backUrl: string | null = null;
  ...
  const metadata = { source: "collx_full_migration_20260811", collx_id: row.collx_id, added: row.added || null,
    category: row.category || null, number: row.number || null, name: row.name || null, team: row.team || null, year: row.year || null,
    brand: row.brand || null, set: row.set || null, flags: row.flags || null, condition: row.condition || null,
    market_value: num(row.market_value), asking_price: num(row.asking_price), purchase_price: num(row.purchase_price),
    original_location: row.location || null, original_notes: row.notes || null, original_quantity: quantity,
    source_front_object: imageEntry.front || null, source_back_object: imageEntry.back || null, migrated_not_for_sale: true, image_errors: imageErrors };
  const description = descriptionFor(row);
  const { data: existingProducts, error: productLookupError } = await supabase.from("products").select("*").eq("store_id", storeId).eq("sku", sku).limit(1);
  if (productLookupError) throw productLookupError;
  let product = existingProducts?.[0] || null; if (product?.ebay_item_id) throw new Error(`Refusing to overwrite eBay-backed product for ${sku}`);
  const productPayload = { store_id: storeId, seller_account_id: null, sku, title, description, price: 0, player: row.name || null,
    sport: row.category || null, quantity, image_url: frontUrl, ebay_item_id: null, last_seen_at: new Date().toISOString() };
  if (product) {
    const { data, error } = await supabase.from("products").update(productPayload).eq("id", product.id).eq("store_id", storeId).select("*").single(); if (error) throw error; product = data;
  } else {
    const { data, error } = await supabase.from("products").insert(productPayload).select("*").single(); if (error) throw error; product = data;
  }
  const { data: existingItems, error: itemLookupError } = await supabase.from("inventory_items").select("*").eq("store_id", storeId).eq("sku", sku).limit(1);
  if (itemLookupError) throw itemLookupError;
  let item = existingItems?.[0] || null;
  const itemPayload = { store_id: storeId, seller_account_id: null, legacy_product_id: product.id, sku, title, description,
    category: row.category || "other_collectable", condition: row.condition || "unknown", status: "draft", quantity,
    cost: num(row.purchase_price), price: asking, currency: "USD", location: row.location || null,
    notes: [row.notes || null, `CollX ${row.collx_id}`, "DRAFT / NOT FOR SALE"].filter(Boolean).join(" | "), metadata,
    updated_at: new Date().toISOString() };
  if (item) {
    const { data, error } = await supabase.from("inventory_items").update(itemPayload).eq("id", item.id).eq("store_id", storeId).select("*").single(); if (error) throw error; item = data;
  } else {
    const { updated_at: _updatedAt, ...insertPayload } = itemPayload;
    const { data, error } = await supabase.from("inventory_items").insert(insertPayload).select("*").single(); if (error) throw error; item = data;
  }
  ...
  if (desired.length) {
    const { data: currentImages, error } = await supabase.from("inventory_images").select("image_url").eq("inventory_item_id", item.id); if (error) throw error;
    const existingUrls = new Set((currentImages || []).map((image) => String(image.image_url)));
    for (const image of desired) if (!existingUrls.has(image.url)) {
      const { error: insertError } = await supabase.from("inventory_images").insert({ inventory_item_id: item.id, image_url: image.url,
        alt_text: `${title} ${image.primary ? "front" : "back"}`, sort_order: image.sort, is_primary: image.primary }); if (insertError) throw insertError;
    }
  }
...
async function mapLimit<T,R>(values: T[], limit: number, fn: (value: T) => Promise<R>) {
  ...
  const workers = Array.from({ length: Math.min(limit, values.length) }, async () => {
    while (true) { const index = cursor++; if (index >= values.length) return; results[index] = await fn(values[index]); }
  });
  await Promise.all(workers); return results;
}

async function verifyMigration(supabase: ReturnType<typeof createSupabaseServerClient>, storeId: string) {
  const classification = await loadClassification(supabase); const products: any[] = []; const items: any[] = [];
  for (let page = 0; page < 20; page += 1) {
    const { data, error } = await supabase.from("products").select("id,sku,image_url,ebay_item_id").eq("store_id", storeId).like("sku", "COLLX-%")
      .range(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE - 1); if (error) throw error;
    const batch = data || []; products.push(...batch); if (batch.length < PAGE_SIZE) break;
  }
  for (let page = 0; page < 20; page += 1) {
  ...
  return { schema: "tcos.collx.full-migration-verification.v1", generatedAt: new Date().toISOString(), sourceRows: classification.sourceRows,
    duplicateCollxRows: Number(classification.summary.duplicateCollxRows || 0), classifiedImportRows: classification.import_collx_ids.length,
    currentCollxProducts: products.length, currentCollxInventoryItems: items.length, missingProducts: missingProducts.length,
    missingItems: missingItems.length, missingProductSample: missingProducts.slice(0,20), missingItemSample: missingItems.slice(0,20),
    activeItems: activeItems.length, collxHostedProductImages, allClassifiedImportsPresent: missingProducts.length === 0 && missingItems.length === 0,
    allImportsNotForSale: activeItems.length === 0 };
}

async function validatedRows(request: Request) {
  const bytes = Buffer.from(await request.arrayBuffer()); const digest = sha256(bytes); if (digest !== SOURCE_SHA) throw new Error(`Source SHA mismatch: ${digest}`);
...
export async function POST(request: Request) {
  try {
    const url = new URL(request.url); const mode = url.searchParams.get("mode") || "classify"; const rows = await validatedRows(request);
    const supabase = createSupabaseServerClient({ admin: true }); const storeId = getActiveStoreId();
    if (mode === "classify") {
      const classification = await classify(rows, supabase, storeId); await saveClassification(supabase, classification);
    ...
    if (mode === "import") {
      const classification = await loadClassification(supabase); const rowById = new Map(rows.map((row) => [row.collx_id, row]));
      const offset = Math.max(0, Math.floor(Number(url.searchParams.get("offset") || 0)));
      const limit = Math.min(250, Math.max(1, Math.floor(Number(url.searchParams.get("limit") || 100))));
      const ids = classification.import_collx_ids.slice(offset, offset + limit); const selected = ids.map((id) => rowById.get(id)).filter(Boolean) as CsvRow[];
      ...
      return NextResponse.json({ success: true, offset, requested: ids.length, completed: results.length, nextOffset, total: classification.import_collx_ids.length,
        frontsCopied: results.filter((result) => result.front).length, backsCopied: results.filter((result) => result.back).length, imageErrors: imageErrors.slice(0,50) });
    }
...
export async function GET() {
  try { const supabase = createSupabaseServerClient({ admin: true }); return NextResponse.json({ success: true, ...(await verifyMigration(supabase, getActiveStoreId())) }); }
  catch (error: any) { return NextResponse.json({ success: false, error: error?.message || String(error) }, { status: 500 }); }
}
