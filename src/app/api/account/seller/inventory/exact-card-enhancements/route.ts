...
function currentTracking(metadata: Record<string, unknown> | null) {
  const tracking = recordValue(recordValue(metadata).instacomp_tracking);
  const current = recordValue(tracking.current);
...
async function runInChunks<T>(
  ...
  for (let index = 0; index < values.length; index += chunkSize) {
    await Promise.all(values.slice(index, index + chunkSize).map(callback));
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const limit = positiveInteger(searchParams.get("limit"), 150, 200) || 150;
    ...
    for (const image of (imageResult.data || []) as InventoryImageRow[]) {
    ...
    const items = inventoryRows.map((row) => {
      ...
      const effectiveCategory = decision.isTradingCard
        ? decision.category
        : row.category || decision.category;

      if (decision.isTradingCard && row.category !== effectiveCategory) {
        repairs.push({
          id: row.id,
      ...
      const imageUrls = uniqueImages([
        ...(imagesByInventoryId.get(row.id) || [])
          .sort(
            (left, right) =>
    ...
    await runInChunks(repairs, 25, async (repair) => {
    ...
    return Response.json(
      { error: error?.message || "Could not enhance seller inventory cards." },
      { status: 500 },
    );
  }
}
