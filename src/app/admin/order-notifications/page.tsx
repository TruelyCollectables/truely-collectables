    ...
    <main className="min-h-screen bg-neutral-100 px-4 py-8 text-neutral-950 sm:px-6">
      <div className="mx-auto max-w-[1500px] space-y-6">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-neutral-300">
                Durable payment, shipment and tracking-update delivery history.
                Sent rows cannot be duplicated; failed rows remain visible and
                retryable.
              </p>
            </div>
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Metric
            label="Sent"
            value={rows.filter((row) => row.status === "sent").length}
          />
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-600">
                Automatic retries are capped at ten attempts. The provider
                message ID is retained as delivery evidence.
              </p>
            </div>
                  ...
                  rows.map((row) => (
                    <tr key={row.id} className="align-top">
                      <td className="px-4 py-4">
                        ...
                        <p className="mt-2 max-w-sm font-medium">
                          {row.subject}
                        </p>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${statusClass(row.status)}`}
                        >
                          {label(row.status)}
                        </span>
                        {row.last_error ? (
                          <p className="mt-2 max-w-xs break-words text-xs text-rose-700">
                            {row.last_error}
                          </p>
                        ) : null}
                      </td>
                      <td className="px-4 py-4">
                        ...
                        <p className="mt-1 text-xs text-neutral-500">
                          Last: {formatDate(row.last_attempt_at)}
                        </p>
                      </td>
                      <td className="px-4 py-4">
