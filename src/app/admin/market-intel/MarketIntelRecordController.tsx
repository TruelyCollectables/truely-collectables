...
function push(router: ReturnType<typeof useRouter>, pathname: string, href: string) {
  if (href.startsWith("#")) {
...
function detailDestination(
  ...
  if (/^\/admin\/market-intel\/purchases\/[^/]+$/.test(pathname)) {
    ...
    if (["CURRENT MARKET / UNIT", "7-DAY MOVE", "SINCE PURCHASE"].includes(label)) {
      const comp = document.querySelector<HTMLAnchorElement>(
        'a[href*="/admin/market-intel/comps/"]',
      );
      return comp?.getAttribute("href") || withHandoff("/admin/market-intel/comps", handoff);
    }
  }
...
export default function MarketIntelRecordController() {
  ...
  useEffect(() => {
    ...
    if (pathname === "/admin/market-intel/watchlist") {
      for (const article of Array.from(document.querySelectorAll<HTMLElement>("article"))) {
        ...
        hint.textContent = "OPEN PLAYER WATCH CENTER →";
        hint.className = "mt-3 text-xs font-black text-cyan-800";
        article.querySelector("h3")?.parentElement?.appendChild(hint);
    ...
    for (const labelNode of Array.from(document.querySelectorAll<HTMLElement>("p"))) {
      ...
      if (card.querySelector("form,button,input,select,textarea")) continue;
      ...
      const click = () => push(router, pathname, destination);
      const keydown = (event: KeyboardEvent) => {
        if (event.key === "Enter" || event.key === " ") {
          event.preventDefault();
          push(router, pathname, destination);
        }
      };
    ...
    for (const row of Array.from(document.querySelectorAll<HTMLTableRowElement>("tbody tr"))) {
      ...
      const click = (event: Event) => {
        if (event.target instanceof HTMLElement && event.target.closest("button,input,select,textarea,a,form")) return;
        push(router, pathname, anchor.href);
      };
      const keydown = (event: KeyboardEvent) => {
        if (event.key === "Enter" || event.key === " ") {
          event.preventDefault();
