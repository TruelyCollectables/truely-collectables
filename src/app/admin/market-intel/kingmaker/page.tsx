      ...
      <div className="relative mx-auto max-w-[1680px] px-4 py-6 sm:px-6 lg:px-8">
        <header className="overflow-hidden rounded-[2rem] border border-white/10 bg-black/70 shadow-2xl backdrop-blur-xl">
          <div className="p-6 lg:p-10">
            ...
            <p className="mt-4 max-w-4xl text-base font-semibold leading-7 text-neutral-400 sm:text-lg">Truely Collectables’ private financial command center for acquisition intelligence, portfolio truth, risk control, and money-making decisions.</p>
          </div>
        ...
        <section className="mt-5 rounded-[1.75rem] border border-amber-300/20 bg-gradient-to-r from-amber-300/[.08] to-emerald-300/[.05] p-6 lg:p-8">
          ...
          <p className="mt-3 max-w-5xl font-semibold leading-7 text-neutral-400">No opportunity becomes owned inventory without a real Purchase Ledger lot. No buy clears without exact identity and verified completed-sale market truth.</p>
        </section>
      </div>
    </main>
  );
...
function Metric({ label, value, good }: { label: string; value: string; good: boolean }) {
  return (
    <div className="bg-white/[.025] px-5 py-5">
      <p className="text-[10px] font-black uppercase tracking-[.18em] text-neutral-500">{label}</p>
      <p className={`mt-2 text-xl font-black ${good ? "text-emerald-300" : "text-amber-300"}`}>{value}</p>
    </div>
  );
}
