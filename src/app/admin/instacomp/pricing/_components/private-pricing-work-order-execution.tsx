...
function payloadFor(row: Row, operation: Operation, draft: Draft) {
  return { attackKey: row.attackKey, expectedVersion: row.version, operation, assignee: draft.assignee.trim() || null, priority: draft.priority, dueAt: draft.dueAt ? new Date(draft.dueAt).toISOString() : null, blockedReason: draft.blockedReason || null, resolutionCode: draft.resolutionCode || null };
}
async function saveExecution(row: Row, operation: Operation, draft: Draft) {
  const response = await fetch("/api/instacomp/pricing/coverage/work-orders/execution", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify(payloadFor(row, operation, draft)) });
...
export default function PrivatePricingWorkOrderExecution() {
  ...
  async function mutate(row: Row, operation: Operation, draft: Draft) {
    ...
    try { await saveExecution(row, operation, draft); setNotice(`${row.releaseYear} ${row.manufacturer} ${row.product} updated.`); await load(); }
  ...
  return <section className="bg-neutral-100 px-4 pb-6 text-neutral-950 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1680px] space-y-4 rounded-[2rem] border border-neutral-900 bg-neutral-950 p-5 text-white shadow-xl lg:p-7">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-300">Tonight&apos;s Listing Operations</p><h2 className="mt-1 text-3xl font-black">Execution Queue</h2><p className="mt-2 max-w-4xl font-semibold text-neutral-300">Find the right work fast, attack the highest unlock first, and apply controls individually or in bulk.</p></div><button type="button" onClick={() => void load()} disabled={loading || bulkSaving} className="rounded-xl bg-amber-300 px-4 py-2.5 font-black text-black disabled:opacity-50">{loading ? "Loading…" : "Reload"}</button></div>
    ...
    <div className="grid gap-2 rounded-2xl border border-white/10 bg-white/5 p-4 lg:grid-cols-[minmax(280px,1fr)_240px_240px_auto]"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search year, manufacturer, product, sport, set, gap, or assignee" className="rounded-xl border border-white/20 bg-neutral-900 px-4 py-3 font-bold text-white placeholder:text-neutral-500"/><select value={lane} onChange={(e) => setLane(e.target.value as Lane | "")} className="rounded-xl border border-white/20 bg-neutral-900 px-3 py-3 font-bold text-white"><option value="">All execution lanes</option>{Object.entries(laneLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select><select value={sort} onChange={(e) => setSort(e.target.value as Sort)} className="rounded-xl border border-white/20 bg-neutral-900 px-3 py-3 font-bold text-white"><option value="unlock_desc">Highest unlock first</option><option value="priority_asc">Priority P1 first</option><option value="due_asc">Due soonest first</option><option value="target_asc">Target A–Z</option></select><div className="rounded-xl border border-amber-300/30 bg-amber-300/10 px-4 py-3"><p className="text-xs font-black uppercase text-amber-200">Visible queue</p><p className="font-black">{rows.length} targets · {visibleUnlock.toLocaleString()} unlock</p></div></div>
