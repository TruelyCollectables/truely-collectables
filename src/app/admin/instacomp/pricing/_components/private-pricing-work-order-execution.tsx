...
type Row = {
  ...
  assignee:string|null; dueAt:string|null; nextReviewAt:string|null; blockedReason:string|null;
  resolutionCode:string|null; updatedAt:string|null; sport:string; releaseYear:string;
  manufacturer:string; product:string; setName:string; gapType:string; potentialUnlock:number;
};
type Report = {
  ...
  pagination:{ limit:number; offset:number; returned:number; totalTargets:number; hasMore:boolean };
  rows:Row[];
};
type Draft = { assignee:string; priority:number; dueAt:string; blockedReason:string; resolutionCode:string };

const laneLabels:Record<Lane,string> = { unassigned:"Unassigned", assigned:"Assigned", overdue:"Overdue", blocked:"Blocked", due_for_review:"Due for review", recently_resolved:"Recently resolved" };
const blockedLabels:Record<string,string> = { missing_checklist:"Missing checklist", missing_pricing_source:"Missing pricing source", identity_conflict:"Identity conflict", insufficient_evidence:"Insufficient evidence", source_access_problem:"Source access problem", other:"Other" };
const resolutionLabels:Record<string,string> = { coverage_fixed:"Coverage fixed", no_action_needed:"No action needed", invalid_target:"Invalid target", more_evidence_required:"More evidence required", dismissed_duplicate:"Duplicate target" };
const quickViews:Array<{value:Lane|"";label:string}> = [{value:"",label:"All"},{value:"unassigned",label:"Unassigned"},{value:"overdue",label:"Overdue"},{value:"blocked",label:"Blocked"},{value:"due_for_review",label:"Due Review"}];
const defaultDraft = ():Draft => ({ assignee:"Truely Collectables Admin", priority:3, dueAt:"", blockedReason:"", resolutionCode:"" });

function localInput(value:string|null){ if(!value)return""; const date=new Date(value); if(Number.isNaN(date.getTime()))return""; const shifted=new Date(date.getTime()-date.getTimezoneOffset()*60000); return shifted.toISOString().slice(0,16); }
function targetLabel(row:Row){ return `${row.releaseYear} ${row.manufacturer} ${row.product} ${row.sport} ${row.setName} ${row.gapType} ${row.assignee||""}`.toLowerCase(); }
function dueValue(value:string|null){ const time=value?new Date(value).getTime():Number.POSITIVE_INFINITY; return Number.isFinite(time)?time:Number.POSITIVE_INFINITY; }
function payloadFor(row:Row,operation:Operation,draft:Draft){ return { attackKey:row.attackKey, expectedVersion:row.version, operation, assignee:draft.assignee.trim()||null, priority:draft.priority, dueAt:draft.dueAt?new Date(draft.dueAt).toISOString():null, blockedReason:draft.blockedReason||null, resolutionCode:draft.resolutionCode||null }; }
async function saveExecution(row:Row,operation:Operation,draft:Draft){ const response=await fetch("/api/instacomp/pricing/coverage/work-orders/execution",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify(payloadFor(row,operation,draft))}); const payload=await response.json().catch(()=>({})); if(!response.ok||payload?.ok!==true)throw new Error(payload?.error||"Execution control could not be saved."); }
...
export default function PrivatePricingWorkOrderExecution(){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const query=new URLSearchParams({limit:String(pageSize),offset:String(offset)}); if(lane)query.set("lane",lane);
      ...
      setReport(payload.execution as Report); setSelected(new Set());
  ...
  function chooseLane(value:Lane|""){ setLane(value); setOffset(0); setSearch(""); }
  function changePageSize(value:number){ setPageSize(value); setOffset(0); }
  function toggleAll(){ setSelected(allSelected?new Set():new Set(rows.map(row=>row.attackKey))); }
  function toggleOne(attackKey:string){ setSelected(current=>{ const next=new Set(current); if(next.has(attackKey))next.delete(attackKey); else next.add(attackKey); return next; }); }
  async function mutate(row:Row,operation:Operation,draft:Draft){ setSaving(row.attackKey); setError(null); setNotice(null); try{ await saveExecution(row,operation,draft); setNotice(`${row.releaseYear} ${row.manufacturer} ${row.product} updated.`); await load(); }catch(cause){ setError(cause instanceof Error?cause.message:"Execution control could not be saved."); }finally{ setSaving(null); } }
  async function mutateBulk(operation:Operation){
    ...
    setBulkSaving(true); setError(null); setNotice(null); const failures:string[]=[]; let completed=0;
    for(const row of selectedRows){ try{ await saveExecution(row,operation,bulkDraft); completed+=1; }catch(cause){ failures.push(`${row.releaseYear} ${row.manufacturer} ${row.product}: ${cause instanceof Error?cause.message:"failed"}`); } }
    if(failures.length)setError(`${completed} updated; ${failures.length} failed. ${failures.slice(0,3).join(" | ")}`); else setNotice(`${completed} work order${completed===1?"":"s"} updated.`);
    await load(); setBulkSaving(false);
  }
  ...
  return <section className="bg-neutral-100 px-4 pb-6 text-neutral-950 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1680px] space-y-4 rounded-[2rem] border border-neutral-900 bg-neutral-950 p-5 text-white shadow-xl lg:p-7">
    ...
    <div className="grid gap-2 rounded-2xl border border-white/10 bg-white/5 p-4 xl:grid-cols-[minmax(280px,1fr)_220px_180px_180px_auto]"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search this page by year, manufacturer, product, sport, set, gap, or assignee" className="rounded-xl border border-white/20 bg-neutral-900 px-4 py-3 font-bold text-white placeholder:text-neutral-500"/><select value={lane} onChange={e=>chooseLane(e.target.value as Lane|"")} className="rounded-xl border border-white/20 bg-neutral-900 px-3 py-3 font-bold text-white"><option value="">All execution lanes</option>{Object.entries(laneLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={sort} onChange={e=>setSort(e.target.value as Sort)} className="rounded-xl border border-white/20 bg-neutral-900 px-3 py-3 font-bold text-white"><option value="unlock_desc">Highest unlock</option><option value="priority_asc">P1 first</option><option value="due_asc">Due soonest</option><option value="target_asc">Target A–Z</option></select><select value={pageSize} onChange={e=>changePageSize(Number(e.target.value))} className="rounded-xl border border-white/20 bg-neutral-900 px-3 py-3 font-bold text-white"><option value={25}>25 per page</option><option value={50}>50 per page</option><option value={100}>100 per page</option><option value={250}>250 per page</option></select><div className="rounded-xl border border-amber-300/30 bg-amber-300/10 px-4 py-3"><p className="text-xs font-black uppercase text-amber-200">Visible page</p><p className="font-black">{rows.length} targets · {visibleUnlock.toLocaleString()} unlock</p></div></div>
    <div className="rounded-2xl border border-amber-300/30 bg-amber-300/10 p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><p className="font-black text-amber-200">Bulk controls</p><p className="text-sm font-bold text-neutral-300">{selectedRows.length} selected from {rows.length} visible</p></div><button type="button" onClick={toggleAll} disabled={!rows.length||bulkSaving} className="rounded-lg border border-white/20 px-3 py-2 text-sm font-black disabled:opacity-40">{allSelected?"Clear visible":"Select visible"}</button></div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-5"><input value={bulkDraft.assignee} onChange={e=>setBulkDraft(d=>({...d,assignee:e.target.value}))} placeholder="Assignee" className="rounded-lg border border-white/20 bg-neutral-900 px-3 py-2.5 font-bold text-white"/><select value={bulkDraft.priority} onChange={e=>setBulkDraft(d=>({...d,priority:Number(e.target.value)}))} className="rounded-lg border border-white/20 bg-neutral-900 px-3 py-2.5 font-bold text-white">{[1,2,3,4,5].map(v=><option key={v} value={v}>P{v}</option>)}</select><input type="datetime-local" value={bulkDraft.dueAt} onChange={e=>setBulkDraft(d=>({...d,dueAt:e.target.value}))} className="rounded-lg border border-white/20 bg-neutral-900 px-3 py-2.5 font-bold text-white"/><select value={bulkDraft.blockedReason} onChange={e=>setBulkDraft(d=>({...d,blockedReason:e.target.value}))} className="rounded-lg border border-white/20 bg-neutral-900 px-3 py-2.5 font-bold text-white"><option value="">No blocker</option>{Object.entries(blockedLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><select value={bulkDraft.resolutionCode} onChange={e=>setBulkDraft(d=>({...d,resolutionCode:e.target.value}))} className="rounded-lg border border-white/20 bg-neutral-900 px-3 py-2.5 font-bold text-white"><option value="">No resolution yet</option>{Object.entries(resolutionLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div><div className="mt-3 flex flex-wrap gap-2"><BulkButton label="Claim selected" disabled={!selectedRows.length||bulkSaving} onClick={()=>void mutateBulk("claim")}/><BulkButton label="Release selected" disabled={!selectedRows.length||bulkSaving} onClick={()=>void mutateBulk("release")}/><BulkButton label="Save controls" disabled={!selectedRows.length||bulkSaving} onClick={()=>void mutateBulk("update")}/><BulkButton label="Resolve selected" disabled={!selectedRows.length||bulkSaving||!bulkDraft.resolutionCode} onClick={()=>void mutateBulk("resolve")}/></div></div>
...
function ExecutionRow({row,selected,saving,onToggle,onMutate}:{row:Row;selected:boolean;saving:boolean;onToggle:(key:string)=>void;onMutate:(row:Row,operation:Operation,draft:Draft)=>Promise<void>}){
  ...
  return <tr className="align-top"><td className="px-4 py-4"><input type="checkbox" checked={selected} onChange={()=>onToggle(row.attackKey)} disabled={saving} aria-label={`Select ${row.releaseYear} ${row.manufacturer} ${row.product}`} className="h-5 w-5"/></td><td className="px-4 py-4"><span className="rounded-full bg-neutral-900 px-3 py-1 text-xs font-black text-white">{laneLabels[row.lane]}</span><p className="mt-2 text-xs font-bold text-neutral-500">{row.status.replaceAll("_"," ")} · P{row.priority}</p></td><td className="px-4 py-4"><p className="font-black">{row.releaseYear} {row.manufacturer} {row.product}</p><p className="mt-1 text-sm font-semibold text-neutral-600">{row.sport} · {row.setName} · {row.gapType.replaceAll("_"," ")}</p></td><td className="px-4 py-4 text-xl font-black">{row.potentialUnlock.toLocaleString()}</td><td className="px-4 py-4"><form onSubmit={submit} className="grid grid-cols-6 gap-2"><input value={draft.assignee} onChange={e=>setDraft(d=>({...d,assignee:e.target.value}))} placeholder="Assignee" className="col-span-2 rounded-lg border px-2 py-2 font-bold"/><select value={draft.priority} onChange={e=>setDraft(d=>({...d,priority:Number(e.target.value)}))} className="rounded-lg border px-2 py-2 font-bold">{[1,2,3,4,5].map(v=><option key={v} value={v}>P{v}</option>)}</select><input type="datetime-local" value={draft.dueAt} onChange={e=>setDraft(d=>({...d,dueAt:e.target.value}))} className="col-span-2 rounded-lg border px-2 py-2 font-bold"/><select value={draft.blockedReason} onChange={e=>setDraft(d=>({...d,blockedReason:e.target.value}))} className="col-span-3 rounded-lg border px-2 py-2 font-bold"><option value="">No blocker</option>{Object.entries(blockedLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><select value={draft.resolutionCode} onChange={e=>setDraft(d=>({...d,resolutionCode:e.target.value}))} className="col-span-3 rounded-lg border px-2 py-2 font-bold"><option value="">No resolution yet</option>{Object.entries(resolutionLabels).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><div className="col-span-6 flex flex-wrap gap-2 pt-1"><button type="button" disabled={saving} onClick={()=>void onMutate(row,"claim",draft)} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-black text-white disabled:opacity-50">Claim</button><button type="button" disabled={saving} onClick={()=>void onMutate(row,"release",draft)} className="rounded-lg border border-neutral-300 px-3 py-2 text-sm font-black disabled:opacity-50">Release</button><button type="submit" disabled={saving} className="rounded-lg bg-black px-3 py-2 text-sm font-black text-white disabled:opacity-50">Save Controls</button><button type="button" disabled={saving||!draft.resolutionCode} onClick={()=>void onMutate(row,"resolve",draft)} className="rounded-lg bg-amber-400 px-3 py-2 text-sm font-black text-black disabled:opacity-40">Record Resolution</button></div></form></td></tr>;
}
