  ...
  const loadActivity = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || payload?.ok !== true) {
        throw new Error(
          payload?.error || "Work-order activity could not be loaded.",
        );
      }
          ...
          >
            <header className="border-b border-neutral-800 bg-neutral-950 p-5 text-white sm:p-7">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-3xl font-black sm:text-4xl">
                    Complete Work-Order Audit Timeline
                  </h2>
                  <p className="mt-2 max-w-4xl font-semibold text-neutral-300">
                    Review creation, reconciliation, review scheduling, claims,
                    releases, control changes, and recorded resolutions without
                    exposing private operator text, assignee labels, target keys,
                    source material, or pricing values.
                  </p>
                </div>
              ...
              <div className="mt-5 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[0.14em]">
                ...
                <HeaderChip text="Full execution history" />
                <HeaderChip text="Admin + system actors" />
                <HeaderChip text="No private controls" />
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              ...
              <section className="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-8">
                ...
                <MetricCard
                  label="Admin"
                  value={report?.summary.adminEvents}
                  loading={loading}
                />
                <MetricCard
                  label="System"
                  value={report?.summary.systemEvents}
                  loading={loading}
                />
                <MetricCard
                  label="Review Events"
                ...
                <MetricCard
                  label="Claims"
                  value={report?.summary.claimedEvents}
                  loading={loading}
                />
                <MetricCard
                  label="Releases"
                  value={report?.summary.releasedEvents}
                  loading={loading}
                />
                <MetricCard
                  label="Resolutions"
                  value={report?.summary.resolutionRecordedEvents}
                  loading={loading}
                />
              </section>
              ...
              <form
                ...
                className="mt-4 rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm"
              >
                <div className="flex flex-wrap items-end gap-3">
                  ...
                  <button
                    type="submit"
                    className="rounded-xl bg-black px-5 py-3 font-black text-white hover:bg-neutral-800"
                  >
                    Apply Filters
                  </button>
                  <button
              ...
              <section className="mt-4 overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-200 p-5">
                  <div>
                    <p className="text-xs font-black uppercase tracking-wider text-neutral-500">
                      Source-Neutral Lifecycle Evidence
                    </p>
                    <h3 className="mt-1 text-2xl font-black">
                      Newest work-order event first
                    </h3>
                  </div>
                  <div className="text-right text-sm font-bold text-neutral-500">
                    <p>
                      {report
                        ? `${formatInteger(firstVisible)}–${formatInteger(lastVisible)} of ${formatInteger(report.pagination.totalEvents)}`
                        : "Loading…"}
                    </p>
                ...
                <div className="divide-y divide-neutral-200">
                  {loading ? (
                    ...
                    report.rows.map((row) => (
                      <ActivityEvent
                        key={`${row.rank}-${row.createdAt}-${row.version}`}
                        row={row}
                      />
                    ))
                  ) : (
                    <div className="px-6 py-14 text-center">
                      ...
                      <p className="mt-2 font-semibold text-neutral-500">
                        Clear or broaden the filters to restore the timeline.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-neutral-200 p-4">
                  ...
                  <p className="text-sm font-bold text-neutral-500">
                    Lifecycle facts only; private controls remain sealed.
                  </p>
                  <button
                    type="button"
...
function ActivityEvent({ row }: { row: ActivityRow }) {
  return (
    <article className="grid gap-4 p-5 hover:bg-neutral-50 lg:grid-cols-[210px_minmax(0,1.4fr)_minmax(0,1fr)_180px] lg:items-center">
      <div>
        ...
        <p className="mt-1 text-xs font-semibold text-neutral-500">
          Version {formatInteger(row.version)}
        </p>
      </div>
      ...
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          />
          <SmallBadge
            text={row.targetActive ? "Active target" : "Inactive target"}
          />
        </div>
      </div>
...
function ActionBadge({ action }: { action: ActivityAction }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-3 py-1.5 text-xs font-black uppercase tracking-wider ${tone}`}
    >
      {actionLabels[action]}
    </span>
  );
...
function LoadingEvent() {
  return (
    <div className="grid gap-4 p-5 lg:grid-cols-[210px_minmax(0,1.4fr)_minmax(0,1fr)_180px]">
