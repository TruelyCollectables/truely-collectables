...
type ReceiptSummary = {
  pendingRows: number;
  receivedAwaitingPhotos: number;
  pendingKnownBasis: number;
};
...
export default function PendingReceiptsClient() {
  const [receipts, setReceipts] = useState<Receipt[]>([]);
  const [summary, setSummary] = useState<ReceiptSummary>({
    pendingRows: 0,
    receivedAwaitingPhotos: 0,
    pendingKnownBasis: 0,
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/api/account/seller/instacomp-pending-receipts?cutoff_date=2026-09-16",
        {
      ...
      if (!response.ok) {
        throw new Error(String(data.error || "Could not load pending receipts."));
      }

      const nextReceipts = Array.isArray(data.receipts) ? data.receipts : [];
      setReceipts(nextReceipts);
      setSummary({
        pendingRows: Number(data.summary?.pendingRows || nextReceipts.length || 0),
    ...
    <main className="px-4 py-7 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70">
          ...
          {loading ? (
            <p className="p-5 font-semibold text-slate-400">Loading receipts…</p>
          ) : receipts.length === 0 ? (
                    ...
                    <tr key={row.acquisitionItemId} className="align-top">
                      <td className="px-4 py-4">
                        <p className="font-black text-white">
                          {row.title || `Acquisition #${row.acquisitionItemId}`}
                        </p>
                        <p className="mt-1 text-xs text-slate-400">
                          {day(row.purchasedAt || row.purchaseDate || row.createdAt)} ·{" "}
                          {row.orderNumber || row.purchaseId}
                        </p>
                        <p className="mt-1 text-xs text-slate-500">
                          {row.cardNumber ? `#${row.cardNumber}` : "Card number pending"} ·{" "}
                          {row.parallel || "Parallel pending"}
                        </p>
                        {row.listingUrl ? (
                          <a
                            href={row.listingUrl}
                            className="mt-2 inline-flex text-xs font-black text-emerald-300 underline"
                          >
                            Open source
                          </a>
                        ) : null}
