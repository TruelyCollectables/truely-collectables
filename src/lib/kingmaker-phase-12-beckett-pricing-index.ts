...
export type BeckettPricingSnapshot = {
  identityKey: string;
...
export function buildBeckettPricingIndex(points: BeckettPricePoint[]) {
  const grouped = new Map<string, BeckettPricePoint[]>();
  ...
  const snapshots: BeckettPricingSnapshot[] = [];
...
export function resolveBeckettPricingSnapshot(index: ReturnType<typeof buildBeckettPricingIndex>, identityKey: string) {
