...
const TRUSTED_WORKFLOW_REF = `${TRUSTED_REPOSITORY}/.github/workflows/import-requested-wnba-checklists.yml@${TRUSTED_REF}`;

type GithubOidcHeader = { alg?: string; kid?: string; typ?: string };
...
async function githubJwksUri() {
  ...
  if (body.issuer !== GITHUB_OIDC_ISSUER || !body.jwks_uri) throw new Error("GitHub OIDC discovery response was invalid.");
...
async function githubJwks(forceRefresh = false) {
  ...
  if (!Array.isArray(body.keys) || body.keys.length === 0) throw new Error("GitHub OIDC signing keys were invalid.");
...
async function verifyToken(token: string) {
  ...
  const claims = decodeJson<GithubOidcClaims>(segments[1]);
  ...
  if (claims.event_name !== "workflow_run") throw new Error("GitHub OIDC event is not trusted.");
  if (claims.workflow_ref !== TRUSTED_WORKFLOW_REF) throw new Error("GitHub OIDC workflow is not trusted.");
  return claims;
}

export async function authenticateRequestedWnbaImportAction(request: Request) {
  const authorization = request.headers.get("authorization") || "";
  const match = authorization.match(/^Bearer\s+(.+)$/i);
  if (!match) throw new Error("Requested WNBA import authorization is required.");
