...
const TRUSTED_WORKFLOW_REF =
  `${TRUSTED_REPOSITORY}/.github/workflows/import-required-wnba-checklists.yml@${TRUSTED_REF}`;
...
type GithubWnbaChecklistOidcClaims = {
  aud?: string | string[];
  exp?: number;
  iat?: number;
  iss?: string;
...
function audienceMatches(audience: GithubWnbaChecklistOidcClaims["aud"]) {
...
async function verifyToken(token: string) {
  ...
  const claims = decodeJson<GithubWnbaChecklistOidcClaims>(segments[1]);
  ...
  if (!claims.event_name || !TRUSTED_EVENTS.has(claims.event_name)) {
    throw new Error("GitHub OIDC event is not trusted.");
  }
...
export async function authenticateWnbaChecklistAction(request: Request) {
  const authorization = request.headers.get("authorization") || "";
  const match = authorization.match(/^Bearer\s+(.+)$/i);
  if (!match) throw new Error("WNBA checklist import authorization is required.");
