...
async function synchronizeImageRows(params: {
  ...
  const finalImages = chooseFinalImages(params.remoteImages, params.existingImages);
  if (!finalImages.length) {
    return { finalImages, added: 0, removed: 0 };
  }
  ...
  for (const [index, imageUrl] of finalImages.entries()) {
...
export async function syncEbayFrontBackImages(params: {
  supabase: SupabaseClient;
  storeId: string;
}) {
  ...
  const allCandidates = inventories.filter((inventory) => {
    ...
    const remoteImages = product
      ...
      : [];
    const currentImages = imagesByInventoryId.get(inventory.id) || [];
    const distinctCurrent = normalizeListingImageUrls(
      currentImages.map((image) => image.image_url),
    );
    const hasSortCollision =
      new Set(currentImages.map((image) => image.sort_order)).size !==
  ...
  let imagesAdded = 0;
  let imagesRemoved = 0;
  let oneImageRemoteListings = 0;
  const errors: Array<{ legacyProductId: number; error: string }> = [];

  await runWorkers(candidates, async (inventory) => {
    const product = productById.get(Number(inventory.legacy_product_id));
    if (!product) return;

    const remoteImages = remote.byItemId.get(String(product.ebay_item_id || "")) || [];
    if (remoteImages.length === 1) oneImageRemoteListings += 1;

    try {
      const syncResult = await synchronizeImageRows({
        supabase: params.supabase,
        inventoryItemId: inventory.id,
        title: product.title || inventory.title,
        remoteImages,
        existingImages: imagesByInventoryId.get(inventory.id) || [],
      });
      const metadata = {
        ...recordValue(inventory.metadata),
        ebay_image_urls: remoteImages,
        ebay_front_back_image_sync_version: IMAGE_SYNC_VERSION,
        ebay_front_back_image_sync_at: checkedAt,
        ebay_front_back_image_count: syncResult.finalImages.length,
      };
      ...
      if (syncResult.finalImages[0]) {
        const { error: productUpdateError } = await params.supabase
          .from("products")
          .update({ image_url: syncResult.finalImages[0] })
          .eq("id", product.id)
          .eq("store_id", params.storeId);
      ...
      updated += 1;
      imagesAdded += syncResult.added;
      imagesRemoved += syncResult.removed;
    } catch (error) {
      errors.push({
        legacyProductId: Number(inventory.legacy_product_id),
        error:
...
export const ebayFrontBackImageSyncTestHelpers = {
  chooseFinalImages,
};
