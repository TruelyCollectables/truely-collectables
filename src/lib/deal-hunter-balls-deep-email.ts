...
export type BallsDeepSurfaceResult = {
  label: string;
  passed: boolean;
  status: number;
  origin: string;
...
export type BallsDeepSummary = {
  ...
  results: BallsDeepSurfaceResult[];
  testedAt: string;
};
...
export async function sendBallsDeepDealHunterEmail(summary: BallsDeepSummary) {
  ...
  const subject = `BALLS DEEP — Deal Hunter LIVE E2E — PASS ${summary.passedCount}/${summary.surfaceCount}`;
  const textRows = summary.results.map(
    (entry) =>
      `PASS — ${entry.label}: query families ${entry.successful}/${entry.families}, failed ${entry.failed}, raw ${entry.raw}, deduped ${entry.deduped}, HTTP ${entry.status}`,
  );
  const text = [
    "BALLS DEEP",
  ...
  return {
    ...
    providerIdPresent: true as const,
