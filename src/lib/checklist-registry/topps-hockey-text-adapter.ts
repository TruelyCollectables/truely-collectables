...
export function parseToppsHockeyTextChecklist(artifact: ChecklistSourceArtifact): ChecklistImportPlan {
  ...
  const releaseSlug = slug(`${parsed.releaseYear}-${parsed.product}-hockey`);
  ...
  const cards = parsed.cards.map((card, index) => {
    ...
    if (!set) throw new Error(`Missing normalized hockey set for ${card.setName}`);
    const type = inferSetType(card.setName);
    return {
      ...
      autographStatus: type === "autograph" ? "autograph" : "non-auto",
      memorabiliaStatus: type === "memorabilia" ? "memorabilia" : "non-memorabilia",
      variation: null,
      sourceNotes: `Topps hockey source line ${card.sourceLine}`,
    };
  });
  const identities = cards.map((card) => {
    const set = sets.find((entry) => entry.sourceKey === card.setSourceKey);
    if (!set) throw new Error(`Missing hockey set for ${card.sourceKey}`);
    return {
  ...
  if (artifact.authority === "official_manufacturer" && !/^https:\/\/(?:www\.)?(?:topps\.com|cdn\.shopify\.com)\//i.test(artifact.sourceUrl)) {
    issues.push({ code: "official_source_domain_mismatch", severity: "error" as const, message: "Official Topps hockey checklist artifacts must originate from topps.com or its official Shopify CDN", rowReference: null });
  }
  const hasErrors = issues.some((issue) => issue.severity === "error");
  return {
    schema: "tcos.checklist.importPlan.v1",
    adapterId: TOPPS_HOCKEY_TEXT_ADAPTER_ID,
    adapterVersion: TOPPS_HOCKEY_TEXT_ADAPTER_VERSION,
    source: {
...
export const toppsHockeyTextChecklistAdapter: ChecklistSourceAdapter = {
  id: TOPPS_HOCKEY_TEXT_ADAPTER_ID,
  version: TOPPS_HOCKEY_TEXT_ADAPTER_VERSION,
  supports(artifact) {
    ...
    return /hockey|nhl/i.test(title) && !/baseball|mlb|football|basketball|soccer/i.test(title);
  },
  parse: parseToppsHockeyTextChecklist,
};
