...
export const UPPER_DECK_HTML_ADAPTER_VERSION = "1.0.0" as const;
...
function splitSubjects(value: string) {
  const cleaned = clean(value).replace(/\s+CL$/i, "");
  return cleaned
    .split(/\s*\/\s*|\s*;\s*|\s+&\s+/)
...
export function parseUpperDeckHtmlChecklist(
  ...
  if (!period.season && !period.releaseYear) {
    issue(
      issues,
      "release_period_missing",
      "error",
      "Upper Deck checklist title must contain a release year or season",
    );
  }
  if (
    ...
    issue(
      ...
      "error",
      "Official Upper Deck artifacts must originate from upperdeck.com/checklist/",
    );
  }
  if (artifact.redistributionAllowed) {
    issue(
      ...
      "warning",
      "Upper Deck source pages should remain privately archived unless redistribution permission is documented",
    );
  }
  ...
  for (const row of rows) {
    ...
    if (!row.cardNumber || !players.length) {
      issue(
        ...
        "error",
        `${row.rawSetName} requires a card number and at least one player/subject`,
        rowReference,
      );
      continue;
    }
    if (row.serial && serialRun == null) {
      issue(
        ...
        "serial_run_unparsed",
        "error",
        `Could not parse Upper Deck serial run '${row.serial}'`,
        rowReference,
      );
      continue;
    }

    const setType = inferSetType({
      setName,
      autographStatus,
    ...
    if (!existingSet) {
      ...
      issue(
        ...
        "set_type_conflict",
        "warning",
        `${setName} appears with both ${existingSet.setType} and ${setType} evidence`,
        rowReference,
      );
    }
    ...
    if (previousOwner && previousOwner !== playerSignature) {
      issue(
        issues,
        "card_number_subject_conflict",
        "error",
        `${setName} #${row.cardNumber} maps to conflicting subjects`,
        rowReference,
      );
      continue;
    }
    ...
    if (descriptor.parallelName) {
      ...
      parallelSourceKey = `${setSourceKey}:${comparable(
        descriptor.parallelName,
      )}:${serialRun || 0}:${comparable(configurationExclusivity)}`;
    ...
    if (fingerprints.has(fingerprint.fingerprintSha256)) {
      issue(
        ...
        "duplicate_identity",
        "error",
        `Duplicate Upper Deck identity for ${row.rawSetName} #${row.cardNumber}`,
        rowReference,
      );
      continue;
    }
  ...
  if (testBatch(html)) {
    issue(
      ...
      "test_batch_only",
      "warning",
      "This HTML fixture proves the Upper Deck adapter but is not a complete checklist page",
    );
  }
