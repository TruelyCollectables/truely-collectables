...
export function assertChecklistPlanComplexity(plan: ChecklistImportPlan) {
  ...
  if (counts.validationIssues > limits.validationIssues) violations.push(`validation issues ${counts.validationIssues}/${limits.validationIssues}`);

  const expansionCeiling = Math.max(1_000, counts.cards * limits.maximumIdentitiesPerCard);
  if (counts.identities > expansionCeiling) violations.push(`identity expansion ${counts.identities}/${expansionCeiling} for ${counts.cards} cards`);

  const serializedBytes = Buffer.byteLength(JSON.stringify(plan), "utf8");
  if (serializedBytes > limits.serializedPlanBytes) violations.push(`normalized plan ${serializedBytes}/${limits.serializedPlanBytes} bytes`);

  if (violations.length) {
    throw new Error(`Checklist import complexity limit exceeded: ${violations.join(", ")}. Split this checklist into smaller validated source files.`);
  }
...
export async function importChecklistArtifact(params: {
  ...
  const content = typeof archiveContent === "string" ? Buffer.from(archiveContent, "utf8") : Buffer.from(archiveContent);
  ...
  if (error) {
    if (uploadedByThisRequest) await supabase.storage.from(CHECKLIST_SOURCE_BUCKET).remove([storage.objectPath]);
