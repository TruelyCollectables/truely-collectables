...
That command is deploy-safe and focused: it runs only the InstaComp™ queue, accuracy, catalog identity, printed-variant identity guard, and 100-card trial scorekeeper simulations without consuming a Vercel deployment. Use `npm run verify:production` for the full lint, shipping, build, guardrail, and GitHub preflight stack.
