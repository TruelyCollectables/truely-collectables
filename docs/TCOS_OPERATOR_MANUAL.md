...
That command prints schema `tcos.instacompTrialIntakeCockpit.v1`, ensures the local `instacomp-trial-inbox/` and `instacomp-trial-images/` folders exist, writes `README_TCOS_INSTACOMP_TRIAL.txt` guide files inside both ignored folders, dry-runs image staging, refreshes the prep/preflight receipts, syncs manifest/worksheet image paths from the current image map, and writes `instacomp-trial-intake.local.json` plus `instacomp-trial-intake.local.md` with one next action. It does not apply staging, delete source files, scan cards, deploy, publish listings, buy postage, create Checkout, call production APIs, approve live money, release payouts, or change runtime switches.
