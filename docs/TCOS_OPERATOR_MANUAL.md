...
- Seller Connections now exposes `1-Click Bring eBay Listings to TCOS` as the primary seller eBay staging action. It stages all remaining connected eBay listings into TCOS review without making anything live automatically. From the staged-row workspace, selected rows can be sent to `InstaComp™` cleanup with their eBay row/title context so card inventory can be scanned/reviewed before becoming TCOS seller drafts. The Ready To Promote and Draft Cleanup cards also expose direct bulk promotion buttons, and the client chunks large promotions into safe 100-row waves so a big active eBay import can move into TCOS seller drafts without silently dropping rows over the API batch cap.
...
- Guardrail compatibility note: next build priority as of July 15, 2026: build `InstaComp™ Multi-Scanner Consensus` before the final 100-card tester pass. That consensus layer is now implemented and remains part of the scanner proof stack while July 16 work moves the top priority to the TCOS Card Knowledge Base MVP.
...
- catalog/checklist evidence coverage: confirmed rows, review-required rows, missing-evidence rows, and coverage percentage
- the exact trial card IDs and fields that missed
- the optional failure-report path, failure-row count, consensus-review-row count, and catalog-review-row count when `--write-failure-report` is used

The trial is considered a pass only when combined identity-and-serial accuracy is at least `94%`, card-identity exact accuracy is at least `94%`, exact serial-number accuracy is at least `94%` when serial-numbered cards are present, every completed row includes timing evidence, average scan speed is `15` seconds per card or faster, p95 scan speed is `45` seconds per card or faster, no manifest row is missing a result, no card still has `review_required` from InstaComp™ Multi-Scanner Consensus, and no row with available catalog evidence is still blocked by catalog/checklist review. Missing catalog evidence is reported as coverage debt so the approved checklist can grow, but a row that already has catalog evidence and still says review-required cannot honestly pass as exact-comp-ready. Critical player, set, card-number, year, and serial-number disagreements must be resolved by checklist/catalog evidence or operator review before the tester can honestly pass.
...
Scheduled seller reconciliation is staggered across two 3-hour schedulers. GitHub Actions workflow `.github/workflows/seller-marketplace-reconciliation.yml` runs at `0 */3 * * *`; native Vercel Pro cron runs 90 minutes later at `30 1-23/3 * * *`. Together they create a seller reconciliation heartbeat about every 1.5 hours while each scheduler still runs only every 3 hours. Both call `/api/cron/seller-ebay-reconciliation` with `Authorization: Bearer {CRON_SECRET}`. The workflow requires GitHub secret `TCOS_CRON_SECRET` to match the production `CRON_SECRET`; optional repository variable `TCOS_RECONCILIATION_URL` can override the default `https://truelycollectables.com/api/cron/seller-ebay-reconciliation`. The current cron endpoint processes one connected seller per invocation and one reconciliation/import batch. Multiple connected sellers may require a faster cadence, more scheduled calls, or a future scaling change.
...
- Seller eBay staging now has an explicit `Send Selected to InstaComp™` handoff plus row-level `Send to InstaComp™` links. `/admin/instacomp` shows a seller eBay staging context banner when opened from those links, reminding the operator to upload/scan the matching fronts/backs, review identity/comps, and then create TCOS seller drafts from cleaned results.
- Admin login failures now return machine-readable codes for missing admin password, invalid password, lockout, and blocked-client states, and the login page displays remaining attempts when available. If the password was changed in `.env.local`, restart the dev server before retrying because Next.js reads env at startup.
