...
Launch Readiness now also includes a Live money runway panel. It mirrors the dedicated live-payment gate summary with approval-blocker, launch-lock, warning, and live Checkout state counts, plus the first live-money next actions. Use it to answer “how much until full live money?” from the main launch dashboard, then open `/admin/live-payment-launch` for the authoritative approval controls and immutable history.

The Live Payment Launch Gate also shows an operator summary, approval-blocker count, launch-lock count, and ordered next-action list. Approval blockers are the remaining payment-readiness checks that must pass before the database approval can be recorded. Launch locks are the two intentional final controls — auditable database approval and `TCOS_LIVE_PAYMENTS_ENABLED` — that keep live Checkout closed until the go-live window even after the rest of the payment stack is ready.
