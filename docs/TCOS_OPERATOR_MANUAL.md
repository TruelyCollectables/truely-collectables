...
`/admin/production-smoke` is the admin-facing production smoke report map. It does not run Vercel or external provider actions; it shows the launch command, clean production target, smoke coverage, common failure meanings, and the manual follow-up links operators should check after `npm run launch:production` succeeds. The smoke suite also verifies the ranked shipping exceptions CSV shape, including the lane that can surface under-$20 seller-protection payout blockers.
