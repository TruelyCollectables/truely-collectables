...
Smoke requests default to a 15-second timeout. Override with `SMOKE_REQUEST_TIMEOUT_MS` if production is slow but still healthy. Failed-check snippets redact key-shaped Stripe, webhook, and JWT values before printing.
