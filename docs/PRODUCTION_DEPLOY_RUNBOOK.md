...
It also runs `npm run check:production-guardrails`, which syntax-checks the production deploy/smoke helpers and verifies the clean production domain cannot be confused with the unwanted `tt3b` alias.
